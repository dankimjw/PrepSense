...
async def search_foods(query: str, limit: int = 20):
    ...
    try:
        ...
        if results:
            ...
            data = [[r['fdc_id'], r['description'][:50], r['brand_owner'] or '', r['category'] or 'Unknown'] for r in results]
            print(tabulate(data, headers=['FDC ID', 'Description', 'Brand', 'Category'], tablefmt='grid'))
        else:
...
async def category_units(category_id: int):
    ...
    try:
        ...
        data = [[u['unit'], f"{u['usage_percentage']:.0f}%", '⭐' if u['is_preferred'] else '', u['unit_type']] for u in units]
