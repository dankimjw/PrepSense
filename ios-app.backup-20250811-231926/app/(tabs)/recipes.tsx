...
interface Recipe {
  ...
  // Backend field names for ingredient counts
...
export default function RecipesScreen() {
  ...
  const handleScroll = (event: any) => {
    ...
    if (currentOffset > threshold && !filtersCollapsed) {
      setFiltersCollapsed(true);
    } else if (currentOffset <= threshold && filtersCollapsed) {
      ...
      <View style={styles.recipeInfo}>
        ...
        <View testID={`recipe-stats-${recipe.id}`} style={styles.recipeStats}>
          <View testID={`have-badge-${recipe.id}`} style={styles.stat}>
            ...
            />
            <Text testID={`have-count-${recipe.id}`} style={styles.statText}>{recipe.usedIngredientCount || recipe.available_count || 0} have</Text>
          </View>
          <View testID={`missing-badge-${recipe.id}`} style={styles.stat}>
            ...
            />
            <Text testID={`missing-count-${recipe.id}`} style={styles.statText}>{recipe.missedIngredientCount || recipe.missing_count || 0} missing</Text>
          </View>
        </View>
      </View>
      <TouchableOpacity
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {/* Simple Filter Row - July 2024 Style */}
      {activeTab === 'discover' && (
