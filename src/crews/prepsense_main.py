...
class PrepSenseMainCrew(Crew):
        ...
        try:
            ...
            if not fresh_items:
                ...
                workflow_result["message"] = (
                    "No fresh ingredients available for recipe recommendations"
                )
            ...
            try:
                ...
                workflow_result["steps"]["recipe_search"] = {"status": "error", "error": str(e)}
                recipes = []
            ...
            try:
                ...
                logger.info(
                    f"Nutrition analysis completed: {len(nutrition_scored_recipes)} recipes analyzed"
                )
            except Exception as e:
            ...
            try:
                preference_scored_recipes = await self.user_preferences.run(
                    nutrition_scored_recipes, user_id
                )
                ...
                logger.info(
                    f"User preference analysis completed: {len(preference_scored_recipes)} recipes scored"
                )
            except Exception as e:
                logger.error(f"User preference analysis failed: {e}")
                workflow_result["steps"]["user_preferences"] = {"status": "error", "error": str(e)}
            ...
            try:
                if self.enable_judge_thyme:
                    ...
                    final_recipes = await self.judge_thyme.run(
                        preference_scored_recipes, max_final_recipes
                    )
                ...
                workflow_result["steps"]["final_judgment"] = {"status": "error", "error": str(e)}
            ...
            try:
                ...
                logger.info(
                    f"Pantry ledger completed: {len(final_recipes_with_pantry)} recipes with pantry matching"
                )
            except Exception as e:
                logger.error(f"Pantry ledger failed: {e}")
                workflow_result["steps"]["pantry_ledger"] = {"status": "error", "error": str(e)}
            ...
            workflow_result.update(
                {
                    ...
                    "workflow_summary": {
                        ...
                        "top_recipe": (
                            final_recipes_with_pantry[0] if final_recipes_with_pantry else None
                        ),
                    },
                }
            )
    ...
    async def _run_recipe_only_workflow(self, input_data: dict) -> dict:
        ...
        if recipes:
            ...
            if self.enable_judge_thyme:
                final_recipes = await self.judge_thyme.run(
                    preference_scored, input_data.get("max_final_recipes", 3)
                )
            else:
                sorted_recipes = sorted(
                    preference_scored, key=lambda x: x.get("combined_score", 0), reverse=True
                )
