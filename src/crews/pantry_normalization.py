...
class PantryNormalizationCrew(Crew):
    ...
    def __init__(self):
        ...
        logger.info(
            f"PantryNormalizationCrew initialized - BiteCam: {self.enable_bite_cam}, FreshFilter: {self.enable_fresh_filter}"
        )
    ...
    async def run(self, input_data: dict):
        ...
        try:
            ...
            if self.enable_fresh_filter:
                ...
                final_items = await self.fresh_filter.run(
                    canonicalized_items, user_id, freshness_days
                )
            else:
