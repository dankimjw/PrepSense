...
class JudgeThyme:
    ...
    async def run(self, preference_scored_recipes: list[dict], max_recipes: int = 5):
        ...
        try:
            ...
            for recipe in preference_scored_recipes:
                ...
                enriched_recipe = {**recipe, **final_judgment, "judge_processed": True}
            ...
            for i, recipe in enumerate(top_recipes):
                recipe["rank"] = i + 1
                recipe["recommendation_strength"] = self._get_recommendation_strength(
                    recipe["final_score"]
                )

            logger.info(
                f"Judge Thyme selected {len(top_recipes)} top recipes from {len(preference_scored_recipes)} candidates"
            )
            return top_recipes
            ...
            fallback_recipes = sorted(
                preference_scored_recipes, key=lambda x: x.get("combined_score", 0), reverse=True
            )[:max_recipes]
    ...
    def _generate_recommendation(
        self, recipe: dict, final_score: float, judgment_factors: dict
    ) -> dict:
