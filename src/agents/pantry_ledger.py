...
class PantryLedger:
    ...
    async def run(self, final_recipes: list[dict], user_id: str, selected_recipe_id: int = None, commit_deduction: bool = False):
        """
        ...
        try:
            ...
            if not pantry_items:
                ...
                return [
                    {**recipe, "pantry_match": {"matches": [], "coverage": 0, "missing_ingredients": recipe.get("ingredients", [])}}
            ...
            for recipe in final_recipes:
                try:
                    ...
                    enriched_recipe = {
                        **recipe,
                        "pantry_match": matching_result
                    }
                    
                    # If this is the selected recipe and commit is requested, perform deduction
                    if (selected_recipe_id and 
                        recipe.get("recipe_id") == selected_recipe_id and 
                        commit_deduction):
                    ...
                    enriched_recipes.append({
                        **recipe,
                        "pantry_match": {"error": str(e), "matches": [], "coverage": 0}
                    })
    ...
    async def _match_recipe_ingredients(self, recipe: dict, pantry_items: list[dict]) -> dict:
        ...
        try:
            ...
            matched_ingredient_names = {match["need"] for match in matches}  # This might need adjustment based on match_and_score output
            missing_ingredients = [
                ing for ing in normalized_recipe_ingredients 
    ...
    async def _perform_pantry_deduction(self, conn, user_id: str, matches: list[dict]) -> dict:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Failed to deduct ingredients: {str(e)}"
            }

    async def _log_pantry_transaction(self, conn, user_id: str, deductions: list[dict], transaction_type: str):
