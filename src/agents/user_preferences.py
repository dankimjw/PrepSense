...
class UserPreferences:
    ...
    async def run(self, nutrition_scored_recipes: list[dict], user_id: str = None):
        ...
        for recipe in nutrition_scored_recipes:
            try:
                ...
                pref_score = preference_result if isinstance(preference_result, (int, float)) else preference_result[0]
                explanation = preference_result[1] if isinstance(preference_result, tuple) else "basic match"
                ...
                combined_score = (nutrition_score * 0.6) + (pref_score * 0.4)  # Weight nutrition slightly higher
    ...
    def _calculate_preference_breakdown(self, recipe: dict, user_prefs: dict) -> dict:
        ...
        if user_cuisines and recipe_cuisines:
            if any(cuisine.lower() in [uc.lower() for uc in user_cuisines] for cuisine in recipe_cuisines):
