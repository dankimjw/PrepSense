...
def process_fao_data():
    ...
    if not all([loss_col, stage_col, commodity_col]):
        logger.error(f"Could not identify required columns. Found: loss={loss_col}, stage={stage_col}, commodity={commodity_col}")
        return
    
    # Filter for consumer and retail stages
    logger.info(f"Filtering for consumer and retail stages...")
    consumer_retail = df[df[stage_col].str.lower().isin(['consumer', 'retail', 'consumption', 'household'])]
    ...
    grouped = consumer_retail.groupby(commodity_col)[loss_col].agg([
