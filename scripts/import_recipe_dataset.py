...
SELECTED_RECIPES_PATH = '/Users/danielkim/_Capstone/PrepSense/Food Data/recipe-dataset-main/final_256_recipes.json'
...
def categorize_recipe(recipe: Dict) -> Dict:
    ...
    if any(word in title or word in ingredients for word in ['breakfast', 'pancake', 'waffle', 'oatmeal', 'toast', 'muffin']):
        meal_type = 'breakfast'
    elif any(word in title or word in ingredients for word in ['lunch', 'sandwich', 'salad', 'soup', 'wrap']):
        meal_type = 'lunch'
...
def process_ingredients(ingredients_str: str) -> List[Dict]:
    ...
    try:
        ...
        return [{'name': ing.strip(), 'amount': '', 'unit': ''} for ing in ingredients if ing.strip()]
...
def import_recipes():
    ...
    local_image_dir = Path('/Users/danielkim/_Capstone/PrepSense/backend_gateway/static/recipe_images')
    ...
    try:
        ...
        for i, recipe in enumerate(selected_recipes):
            ...
            recipe_data = {
                'id': recipe_id,
                'title': recipe['title'],
                'image': image_url or f"https://via.placeholder.com/400x300?text={recipe['title'][:20]}",
                'imageType': 'jpg',
                'servings': 4,
                'readyInMinutes': 45,
                ...
                'analyzedInstructions': [{
                    'steps': [{'step': step.strip()} for step in recipe['instructions'].split('.') if step.strip()]
                }],
