...
class BackupRecipeSystemSetup:
    ...
    async def run_full_setup(self, test_mode: bool = False, max_import_records: Optional[int] = None):
        ...
        try:
            ...
            if test_mode:
                ...
                logger.info(f"🧪 Running in test mode - importing only {max_import_records} records")
...
async def main():
    ...
    parser.add_argument('--test-mode', action='store_true', 
                       help='Run in test mode (import only 100 records)')
    parser.add_argument('--max-records', type=int, 
                       help='Maximum number of records to import')
    parser.add_argument('--database-only', action='store_true',
                       help='Only setup database schema (no data import)')
    parser.add_argument('--import-only', action='store_true',
                       help='Only import data (skip database setup)')
    ...
    if args.database_only:
        ...
        await setup.run_full_setup(
            ...
            max_import_records=args.max_records
        )
