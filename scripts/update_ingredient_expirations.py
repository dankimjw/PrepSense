...
def update_ingredient_expirations():
    ...
    expiration_updates = [
        ...
        {
            "ingredients": ["eggs", "Greek yogurt", "mozzarella cheese", "tomatoes", "cucumber", "lemon", "lime"],
            "days_until_expiry": 7,
            "quantity_multiplier": 0.8
        },
        # 14 days from now (okay - green)
        {
            "ingredients": ["chicken thighs", "chicken drumsticks", "butter", "heavy cream", "ricotta cheese"],
            ...
            for ingredient_name in update_group["ingredients"]:
                if ingredient_name in all_items:
                    ...
                    if ingredient_name in base_quantities:
                        ...
                        """, (round(new_quantity, 2), expected_unit, expiration_date, item['pantry_item_id']))
                    else:
                    ...
                    display_unit = expected_unit if ingredient_name in base_quantities else item['unit_of_measurement']
                    print(f"Updated {ingredient_name}: {new_quantity:.2f} {display_unit} - expires in {days} days")
...
def verify_updates():
    ...
    try:
        ...
        for item in cur.fetchall():
            ...
            days = days_diff.days
            status = "🔴 CRITICAL" if days <= 1 else "🟠 WARNING" if days <= 3 else "🟡 CAUTION" if days <= 7 else "🟢 GOOD"
            print(f"{item['product_name']:25} {item['quantity']:6.2f} {item['unit_of_measurement']:10} | {days:2d} days | {status}")
