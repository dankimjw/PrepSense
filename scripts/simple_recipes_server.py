...
MOCK_RECIPES = [
    ...
    },
    {
        ...
        "instructions": ["Season salmon", "Heat grill", "Cook 6 minutes per side", "Serve with lemon"],
        "readyInMinutes": 25,
        "servings": 2,
    ...
    def do_GET(self):
        ...
        if path == '/health':
            self.send_json_response({
                "status": "healthy",
                "service": "Simple Recipes API Server",
                "port": 8001
            })
            ...
            self.send_json_response({
                "recipes": recipes,
                "message": f"Random selection of {len(recipes)} recipes"
            })
