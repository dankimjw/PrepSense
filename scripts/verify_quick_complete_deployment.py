...
class QuickCompleteVerifier:
    ...
    async def test_check_ingredients_endpoint(self) -> bool:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/recipe-consumption/check-ingredients", json=payload
            ) as response:
                duration = time.time() - start_time
                ...
                if response.status == 200:
                    ...
                    if "ingredients" in data and isinstance(data["ingredients"], list):
                        ...
                        if data["ingredients"]:
                            ...
                            missing_fields = [
                                field for field in required_fields if field not in ingredient
                            ]
                    ...
                    self.log_test(
                        "Check Ingredients Endpoint", False, f"HTTP {response.status}", duration
                    )
                    return False
        except Exception as e:
    ...
    async def test_quick_complete_endpoint(self) -> bool:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/recipe-consumption/quick-complete", json=payload
            ) as response:
                duration = time.time() - start_time
                ...
                if response.status == 200:
                    ...
                    self.log_test(
                        "Quick Complete Endpoint", False, f"HTTP {response.status}", duration
                    )
                    return False
        except Exception as e:
    ...
    async def test_endpoint_performance(self) -> bool:
        ...
        try:
            payload = {"user_id": 111, "recipe_id": 716429, "servings": 1}
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/pantry/user/111/items") as response:
                ...
                if response.status == 200:
                    ...
                    self.log_test(
                        "Pantry Service Integration", False, f"HTTP {response.status}", duration
                    )
                    return False
        except Exception as e:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        print("-" * 80)
        print(
            f"📊 RESULTS: {self.results['tests_passed']}/{self.results['tests_run']} tests passed "
