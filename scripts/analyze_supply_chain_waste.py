...
def analyze_supply_chain_waste():
    ...
    commodity_stages = df.groupby(['commodity', 'food_supply_stage'])['loss_percentage'].agg(['mean', 'median', 'count'])
    ...
    for commodity in commodities[:50]:  # Top 50 commodities
        stages = df[df['commodity'] == commodity].groupby('food_supply_stage')['loss_percentage'].median()
        ...
        if len(stages) > 0:
            ...
            stage_order = ['Farm', 'Harvest', 'Post-harvest', 'Storage', 'Processing', 
                          'Transport', 'Wholesale', 'Retail', 'Households']
    ...
    sorted_commodities = sorted(cumulative_losses.items(), 
                               key=lambda x: x[1]['cumulative_loss_pct'], 
                               reverse=True)
