    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            for row in results:
                ...
                first_cooked = row['first_cooked'].strftime('%Y-%m-%d') if row['first_cooked'] else 'Never'
                last_cooked = row['last_cooked'].strftime('%Y-%m-%d') if row['last_cooked'] else 'Never'
            ...
            print(tabulate(
                table_data,
                headers=['User ID', 'Name', 'Email', 'Completed', 'Favorites', 'Liked', 'First Cooked', 'Last Cooked'],
                tablefmt='grid'
            ))
            ...
            if users_with_completed > 0:
                print(f"  Average recipes per active user: {total_completed / users_with_completed:.1f}")
            ...
            if has_recipe_history:
                ...
                print(tabulate(
                    table_data,
                    headers=['User ID', 'Name', 'History Entries'],
                    tablefmt='grid'
                ))
            else:
            ...
            if results:
                ...
                for row in results:
                    ...
                    table_data.append([
                        row['user_name'],
                        row['recipe_title'][:40] + '...' if len(row['recipe_title']) > 40 else row['recipe_title'],
                        row['source'],
                        row['rating'],
                        cooked_date
                    ])
