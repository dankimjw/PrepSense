...
CATEGORY_MAPPINGS = {
    'produce': [
        ...
        'apples', 'bananas', 'oranges', 'lemons', 'limes', 'peaches', 'pears',
        'carrots', 'celery', 'lettuce', 'spinach', 'broccoli', 'tomatoes',
        'potatoes', 'onions', 'garlic', 'peppers', 'cucumber', 'kale'
    ],
    'dairy': [
        'milk', 'yogurt', 'cheese', 'butter', 'cream', 'sour cream',
        'cottage cheese', 'cream cheese', 'half and half', 'eggs'
    ],
    'meat': [
        'chicken', 'beef', 'pork', 'turkey', 'bacon', 'sausage',
        'ground beef', 'steak', 'ham', 'lamb', 'fish', 'salmon', 'tuna'
    ],
    'beverages': [
        'juice', 'soda', 'water', 'coffee', 'tea', 'beer', 'wine',
        'energy drink', 'sports drink', 'kombucha', 'coconut water'
    ],
    'grains': [
        'bread', 'rice', 'pasta', 'cereal', 'oats', 'quinoa', 'flour',
        'crackers', 'tortillas', 'bagels', 'muffins', 'granola'
    ],
    'spices': [
        'salt', 'pepper', 'cinnamon', 'paprika', 'oregano', 'basil',
        'thyme', 'rosemary', 'garlic powder', 'onion powder', 'cumin'
    ]
}
        ...
        for fix in category_fixes:
            if dry_run:
                ...
                await conn.execute(
                    ...
                    fix['suggested_category'], fix['id']
                )
                logger.info(
                    f"Categorized: {fix['name']} → {fix['suggested_category']}"
                )
            fixed_count += 1
...
async def main():
    ...
    parser.add_argument('--categories-only', action='store_true', help='Only fix categories, not units')
