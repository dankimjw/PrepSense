...
class PrepSenseTestSuite:
    ...
    def test_database_connectivity(self) -> bool:
        ...
        try:
            ...
            if response.status_code in [401, 403]:
                ...
                self.record_result(
                    test_name, True, f"Unexpected but not failure: {response.status_code}"
                )
                return True
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if chat_data["response"] and len(chat_data["response"]) > 10:
                    self.log(
                        f"Chat system working - got {len(chat_data.get('recipes', []))} recipes",
                        "pass",
                        test_name,
                    )
                    self.record_result(
                        test_name, True, f"Response length: {len(chat_data['response'])}"
                    )
                    return True
                else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(recipes_data, dict) and "recipes" in recipes_data:
                    ...
                    self.log(
                        f"Unexpected recipe data structure: {type(recipes_data)}", "fail", test_name
                    )
                ...
                if len(recipes) > 0:
                    ...
                    missing_fields = [
                        field for field in required_fields if field not in sample_recipe
                    ]
                    ...
                    self.log(
                        f"Recipe system working - found {len(recipes)} demo recipes",
                        "pass",
                        test_name,
                    )
    ...
    def test_pantry_management(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log(
                    f"Pantry endpoint accessible - found {len(items_data)} items", "pass", test_name
                )
                ...
                self.record_result(
                    test_name, True, f"Unexpected but not critical: {response.status_code}"
                )
                return True
    ...
    def test_image_generation(self) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/v1/chat/generate-recipe-image", json=image_payload, timeout=15
            )
                ...
                if "image_url" in image_data and image_data["image_url"]:
                    ...
                    try:
                        ...
                        if img_response.status_code == 200:
                            ...
                            self.record_result(
                                test_name, True, f"Generated: {image_data['image_url']}"
                            )
                            return True
                        else:
    ...
    def test_user_workflow_recipe_discovery(self) -> bool:
        ...
        try:
            ...
            image_response = self.session.post(
                ...
                json={"recipe_name": recipe_name, "use_generated": False},
                timeout=15,
            )
    ...
    def test_error_handling(self) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/v1/chat/message", json={"invalid": "data"}, timeout=10
            )
            ...
            response = self.session.post(
                f"{self.base_url}/api/v1/chat/message",
                json={"message": large_message, "user_id": 111},
                timeout=15,
            )
    ...
    def test_performance_basics(self) -> bool:
        ...
        try:
            ...
            acceptable_performance = not any(
                "very slow" in result for result in performance_results
            )
    ...
    def run_comprehensive_tests(self) -> Tuple[bool, Dict]:
        ...
        test_categories = {
            "Infrastructure": [
                ...
                (
                    "External API Config",
                    lambda: all(self.test_external_api_configuration().values()),
                ),
            ],
            "Core Features": [
            ...
            "User Workflows": [("Recipe Discovery Flow", self.test_user_workflow_recipe_discovery)],
            "System Quality": [
        ...
        for category, tests in test_categories.items():
            ...
            print(
                f"{status_color}{status_emoji} {category}: {category_passed}/{category_total} ({success_rate:.1f}%){Colors.RESET}"
            )
        ...
        print(
            f"{status_color}{status_emoji} {status_text}: {overall_passed}/{overall_total} tests passed ({overall_success_rate:.1f}%){Colors.RESET}"
        )
        ...
        if "database_connectivity" in [
            r for r, data in self.test_results.items() if not data["passed"]
        ]:
