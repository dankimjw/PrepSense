...
class MessageContextService:
    ...
    def __init__(self):
        ...
        self.meal_patterns = {
            'breakfast': [
                'breakfast', 'morning', 'brunch', 'wake up', 'start my day',
                'morning meal', 'first meal', 'break fast'
            ],
            'lunch': [
                'lunch', 'midday', 'noon', 'work meal', 'quick bite',
                'afternoon meal', 'lunchtime', 'mid-day'
            ],
            'dinner': [
                'dinner', 'supper', 'evening', 'tonight', 'family meal',
                'evening meal', 'main meal', 'night meal'
            ],
            'snack': [
                'snack', 'appetizer', 'munchies', 'bite', 'treat',
                'nibble', 'finger food', 'small bite', 'between meals'
            ],
            'dessert': [
                'dessert', 'sweet', 'cake', 'cookie', 'after dinner',
                'sweet treat', 'pudding', 'ice cream', 'pie'
            ]
        }
        ...
        self.time_patterns = {
            'quick': [
                'quick', 'fast', 'hurry', 'instant', 'speedy',
                'rapid', 'swift', 'express', 'asap', 'short time',
                'in a rush', 'pressed for time', 'minimal time'
            ],
            'medium': [
                'normal', 'regular', 'standard', 'typical',
                'usual', 'moderate time', 'reasonable time'
            ],
            'leisurely': [
                'slow', 'weekend', 'elaborate', 'fancy', 'special',
                'take my time', 'no rush', 'lazy', 'relaxed',
                'sunday', 'holiday', 'celebration'
            ]
        }
        ...
        self.health_patterns = {
            'healthy': [
                'healthy', 'nutritious', 'light', 'fresh', 'clean',
                'wholesome', 'balanced', 'good for me', 'diet friendly',
                'low fat', 'low sodium', 'heart healthy'
            ],
            'comfort': [
                'comfort', 'hearty', 'filling', 'cozy', 'satisfying',
                'soul food', 'homestyle', 'rich', 'indulgent',
                'warming', 'stick to your ribs'
            ],
            'diet': [
                'diet', 'low calorie', 'weight loss', 'lean', 'fit',
                'slim', 'light', 'calorie conscious', 'portion control',
                'watching my weight', 'cutting calories'
            ]
        }
    ...
    def _extract_ingredient_focus(self, message: str) -> List[str]:
        ...
        exclude_words = {
            'something', 'anything', 'recipe', 'meal', 'dish', 'food',
            'breakfast', 'lunch', 'dinner', 'snack', 'quick', 'easy',
            'healthy', 'delicious', 'tasty', 'good', 'nice', 'great'
        }
    ...
    def _extract_serving_info(self, message: str) -> Dict:
        ...
        if any(phrase in message for phrase in ['just for me', 'single serving', 'one person', 'myself']):
    ...
    def _extract_special_requirements(self, message: str) -> Dict:
        ...
        if any(phrase in message for phrase in [
            'no shopping', 'dont buy', "don't buy", 'only what i have',
            'without shopping', 'no store', 'already have'
        ]):
        ...
        if any(phrase in message for phrase in ['one pot', 'one pan', 'single pot', 'easy cleanup']):
    ...
    def _detect_expiring_intent(self, message: str) -> bool:
        ...
        expiring_phrases = [
            ...
            'spoil', 'use up', 'before it goes bad', 'need to use'
        ]
