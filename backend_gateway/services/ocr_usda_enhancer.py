...
class OCRUSDAEnhancer:
    ...
    async def enhance_ocr_results(self, ocr_items: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in ocr_items:
            try:
                ...
                enhanced_items.append({**item, "enhancement_error": str(e), "enhanced": False})
    ...
    async def _enhance_single_item(self, item: dict[str, Any]) -> dict[str, Any]:
        ...
        if barcode:
            barcode_results = await self.usda_service.search_foods(barcode=barcode, limit=1)

            if barcode_results:
                return self._build_enhanced_item(
                    item, barcode_results[0], confidence=1.0, match_type="barcode"
                )

        # Fall back to text search
        search_results = await self.usda_service.search_foods(query=cleaned_name, limit=5)

        if search_results:
            # Find best match
            best_match, confidence = self._find_best_match(cleaned_name, search_results)

            if best_match and confidence > 0.6:  # Threshold for acceptance
                return self._build_enhanced_item(
                    item, best_match, confidence=confidence, match_type="text"
                )

        # No good match found
        return {**item, "cleaned_name": cleaned_name, "enhanced": False, "no_match_found": True}
    ...
    def _find_best_match(
        ...
        for result in results:
            ...
            desc_score = SequenceMatcher(None, query.lower(), result["description"].lower()).ratio()

            # Bonus for brand match if present
            if result.get("brand_info"):
                brand_score = (
                    SequenceMatcher(None, query.lower(), result["brand_info"].lower()).ratio() * 0.3
                )  # Weight brand less
