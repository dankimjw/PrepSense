...
class PracticalFoodCategorizationService:
    ...
    def __init__(self, db_service=None):
        ...
        self.validation_rules = {
            ...
            "meat_no_volume": {
                "condition": lambda item: any(
                    meat in item.lower() for meat in ["chicken", "beef", "pork", "fish", "meat"]
                ),
            ...
            "produce_count_no_volume": {
                "condition": lambda item: any(
                    fruit in item.lower()
                    for fruit in ["apple", "banana", "orange", "potato", "onion"]
                ),
                "forbidden_units": ["ml", "l", "fl oz", "gallon", "pint", "quart"],
    ...
    async def _categorize_with_spoonacular(self, item_name: str) -> Optional[FoodCategorization]:
        ...
        try:
            ...
            if parsed and len(parsed) > 0:
                ...
                category_info = self.category_patterns.get(
                    our_category, self.category_patterns["produce_countable"]
                )
    ...
    def _categorize_with_enhanced_patterns(self, item_name: str) -> FoodCategorization:
        ...
        if category_scores:
            ...
            confidence = min(
                0.7, 0.3 + (best_info["score"] * 0.1)
            )  # Scale confidence based on matches
    ...
    def _map_spoonacular_category(self, spoon_category: str, item_name: str) -> str:
        ...
        category_mapping = {
            ...
            "condiment": (
                "liquids"
                if any(liquid in item_name.lower() for liquid in ["sauce", "dressing", "syrup"])
                else "dry_goods"
            ),
            "baking": "dry_goods",
    ...
    async def validate_unit_for_item(self, item_name: str, unit: str) -> dict[str, Any]:
        ...
        for rule_name, rule in self.validation_rules.items():
            if rule["condition"](item_name) and unit_lower in [
                u.lower() for u in rule["forbidden_units"]
            ]:
