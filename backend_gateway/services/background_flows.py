    ...
    async def run_preference_vector_builder(self, user_id: int, db_service) -> UserPreferenceVector:
        ...
        try:
            ...
            try:
                ...
                history = db_service.execute_query(
                    history_query, {"user_id": user_id, "since_date": since_date}
                )
            except Exception as e:
                logger.warning(
                    f"Could not load interaction history (table may not exist): {str(e)}"
                )
                history = []
            ...
            if interaction["action"] in ["rated_positive", "cooked", "saved"]:
                ...
                if "ingredients" in metadata:
                    for ingredient in metadata["ingredients"]:
                        ingredient_preferences[ingredient] = (
                            ingredient_preferences.get(ingredient, 0) + 0.1
                        )
...
class CacheManager:
    ...
    async def ensure_fresh_cache(self, user_id: int, db_service, force_refresh: bool = False):
        ...
        if force_refresh or not self.flow_manager.is_cache_fresh(
            self.flow_manager.inventory_cache, 6
        ):
        ...
        if force_refresh or not self.flow_manager.is_cache_fresh(
            self.flow_manager.preference_cache, 24
        ):
