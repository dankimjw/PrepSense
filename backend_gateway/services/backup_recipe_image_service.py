...
class BackupRecipeImageService:
    ...
    async def get_optimized_image_path(self, image_name: Optional[str]) -> Optional[Path]:
        ...
        if (cached_path.exists() and 
            cached_path.stat().st_mtime > original_path.stat().st_mtime):
            return cached_path
        ...
        try:
            ...
            for image_path in self.images_dir.iterdir():
                if (image_path.is_file() and 
                    any(image_path.name.lower().endswith(ext) for ext in SUPPORTED_FORMATS)):
                    images.append({
        ...
        try:
            for cache_file in self.cache_dir.iterdir():
                if (cache_file.is_file() and 
                    cache_file.stat().st_mtime < cutoff_time):
                    cache_file.unlink()
                    cleaned_count += 1
...
async def serve_backup_recipe_image(image_name: str, optimized: bool = True) -> Tuple[Optional[Path], dict]:
    """
