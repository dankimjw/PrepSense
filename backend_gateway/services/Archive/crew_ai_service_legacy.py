...
class CrewAIService:
    ...
    async def process_message(self, user_id: int, message: str, use_preferences: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("\n💾 STEP 4: Checking recipe cache...")
            cache_stats = self.recipe_cache.get_cache_stats(user_id, valid_items)
            logger.info(f"📊 Cache stats: {cache_stats}")
            
            cached_recipes = self.recipe_cache.get_cached_recipes(user_id, valid_items)
            
            if cached_recipes:
                logger.info(f"✅ Found {len(cached_recipes)} recipes from cache")
                all_recipes = cached_recipes
            else:
                logger.info("❌ Cache miss - fetching fresh recipes")
                
                # Step 5: Get matching saved recipes
                logger.info("\n📚 STEP 5: Checking saved recipes...")
                ...
                # Step 6: Get MORE Spoonacular recipes to build cache
                logger.info(f"\n🥄 STEP 6: Fetching 40 Spoonacular recipes for cache...")
                spoonacular_recipes = await self._get_spoonacular_recipes(valid_items, message, user_preferences, 40)
                logger.info(f"✅ Found {len(spoonacular_recipes)} Spoonacular recipes")
                
                # Step 7: Combine all recipes
                logger.info("\n🔀 STEP 7: Combining recipe sources...")
                ...
                # Cache all recipes
                self.recipe_cache.cache_recipes(user_id, valid_items, all_recipes)
                
                # Get subset for this response
                cached_recipes = self.recipe_cache.get_cached_recipes(user_id, valid_items)
                all_recipes = cached_recipes if cached_recipes else all_recipes[:5]
