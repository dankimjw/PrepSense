...
class PantryItemManagerEnhanced(PantryItemManager):
    ...
    async def add_items_batch_enhanced(self, user_id: int, items: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    async def validate_and_fix_existing_items(self, user_id: int, 
                                            fix_units: bool = True,
                                            fix_categories: bool = True) -> Dict[str, Any]:
        """
    ...
    async def smart_add_item(self, user_id: int, item_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['saved_count'] > 0:
                ...
                return {
                    'success': False,
                    'error': result.get('errors', ['Failed to add item'])[0]
                }
            ...
            if match2:
                ...
                if brand_match:
                    ...
                    if potential_brand[0].isupper() and (
                        "'" in potential_brand or 
                        potential_brand in ['Trader Joe', 'Great Value', 'Kirkland', 'Store Brand']
                    ):
