...
class StartupImageService:
                ...
                try:
                    ...
                    if existing_record and existing_record.get('gcs_signed_url'):
                        ...
                        if expires_at and expires_at > datetime.now().astimezone():
                            ...
                            return {"recipe_id": recipe_id, "status": "already_exists", "action": "skipped"}
                    ...
                    try:
                        ...
                        ingredients = [ing.get('name', '') for ing in recipe_data.get('extendedIngredients', [])[:3]]
                    except Exception as e:
                    ...
                    if image_url:
                        ...
                        return {"recipe_id": recipe_id, "status": "generated", "title": recipe_title, "url": image_url}
                    else:
    ...
    def _print_summary(self, results: Dict[str, Any]):
        ...
        if url_stats:
            print(f"🔄 URL Refresh: {url_stats.get('refreshed', 0)} refreshed, {url_stats.get('failed', 0)} failed")
        
        # Generation summary
        gen_stats = results.get("priority_generation", {})
        if gen_stats:
            print(f"🎨 Image Generation: {gen_stats.get('generated', 0)} new, {gen_stats.get('already_exists', 0)} existing")
        
        # Database statistics
        db_stats = results.get("statistics", {}).get("database", {})
        if db_stats:
            print(f"💾 Database: {db_stats.get('total_images', 0)} total, {db_stats.get('valid_urls', 0)} valid URLs")
