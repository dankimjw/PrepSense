...
class UserService:
    def __init__(self, bq_client=None):
        self.bq_client = None
        self.table_id = None
        self.profile_table_id = None # Added for the 'user' (singular) table
        ...
        if bq_client:
            ...
            try:
                ...
                self.user_preferences_table_id = f"{settings.BIGQUERY_PROJECT}.{settings.BIGQUERY_DATASET}.user_preference"
                
                self._ensure_table_exists()
                self._ensure_profile_table_exists() # Call to ensure the new table
                self._ensure_user_preference_table_exists()
