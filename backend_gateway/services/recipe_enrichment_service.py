...
class RecipeEnrichmentService:
    ...
    def enrich_recipe(self, recipe: dict[str, Any], user_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("✅ Recipe enriched with Spoonacular-style structure")
    ...
    def _normalize_basic_fields(self, recipe: dict[str, Any]) -> dict[str, Any]:
        ...
        if "image" not in recipe:
            recipe["image"] = (
                f"https://via.placeholder.com/556x370.png/40E0D0/000000?text={recipe['title'][:20]}"
            )
    ...
    def _normalize_ingredient_dict(
        self, ingredient: dict[str, Any], ingredient_id: int
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "original": ingredient.get(
                "original", ingredient.get("name", ingredient.get("ingredient_name", "Unknown"))
            ),
    ...
    def _structure_instructions(self, recipe: dict[str, Any]) -> dict[str, Any]:
                    ...
                    steps.append(
                        {
                            ...
                            "step": instruction.get(
                                "step", instruction.get("instruction", str(instruction))
                            ),
        ...
        recipe["analyzedInstructions"] = [{"name": "", "steps": steps}]
    ...
    def _add_spoonacular_fields(self, recipe: dict[str, Any]) -> dict[str, Any]:
        ...
        if "summary" not in recipe:
            ...
            recipe["summary"] = (
                f"A delicious recipe with {ingredients_count} ingredients, ready in {time} minutes. Perfect for a satisfying meal!"
            )
    ...
    def enrich_saved_recipes_batch(
        self, recipes: list[dict[str, Any]], user_id: int
    ) -> list[dict[str, Any]]:
