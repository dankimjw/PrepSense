...
class RecipeDeduplicationService:
    ...
    def normalize_ingredient_name(self, ingredient: str) -> str:
        ...
        ingredient = re.sub(
            r"\b\d+.*?(oz|lb|g|kg|ml|l|cup|tbsp|tsp|pound|ounce|gram|kilogram|liter|milliliter)\b",
            "",
            ingredient,
            flags=re.IGNORECASE,
        )
        ingredient = re.sub(
            r"\b(organic|fresh|frozen|dried|chopped|minced|sliced|diced|crushed|ground)\b",
            "",
            ingredient,
            flags=re.IGNORECASE,
        )
    ...
    def generate_recipe_fingerprint(self, recipe: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Error generating fingerprint for recipe {recipe.get('id', 'unknown')}: {str(e)}"
            )
    ...
    def is_duplicate(
        ...
        logger.debug(
            f"Duplicate check: '{recipe1.get('title', '')}' vs '{recipe2.get('title', '')}' "
    ...
    def deduplicate_recipes(
        ...
        for i, recipe in enumerate(recipes):
            ...
            for unique_recipe in unique_recipes:
                if self.is_duplicate(recipe, unique_recipe, threshold):
                    ...
                    logger.debug(
                        f"Recipe {recipe_id} '{recipe.get('title', '')}' "
        ...
        logger.info(
            f"Deduplication complete: {len(unique_recipes)} unique recipes, "
