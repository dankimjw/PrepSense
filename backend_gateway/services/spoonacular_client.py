...
class SpoonacularClient:
    ...
    def __init__(
        self, api_key: Optional[str] = None, base_url: str = "https://api.spoonacular.com"
    ):
        """
        ...
        self.client = httpx.Client(
            base_url=base_url, timeout=30.0, headers={"User-Agent": "PrepSense/1.0"}
        )
    ...
    def get_recipe_information(
        ...
        """
        params = {"includeNutrition": str(include_nutrition).lower(), "apiKey": self.api_key}
    ...
    def get_similar_recipes(self, recipe_id: int, number: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        params = {"number": number, "apiKey": self.api_key}
    ...
    def substitute_ingredients(self, ingredient_name: str) -> Dict[str, Any]:
        ...
        """
        params = {"ingredientName": ingredient_name, "apiKey": self.api_key}

        return self._make_request(
            method="GET", endpoint="/food/ingredients/substitutes", params=params
        )
        ...
        while retries <= retry_count:
            try:
                response = self.client.request(
                    method=method, url=endpoint, params=params, json=data
                )
