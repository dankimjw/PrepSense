...
class EnvironmentalImpactService:
    ...
    def process_impact_data(self) -> Dict[str, Dict]:
        ...
        for owid_product, food_items in self.OWID_TO_FOOD_MAPPING.items():
            ...
            metrics = {
                'ghg_kg_co2e_per_kg': self._get_metric(datasets.get('ghg'), owid_product, 'per kilogram'),
                'land_m2_per_kg': self._get_metric(datasets.get('land'), owid_product, 'per kilogram'),
                'water_L_per_kg': self._get_metric(datasets.get('water'), owid_product, 'per kilogram'),
                'eutrophying_g_per_kg': self._get_metric(datasets.get('eutrophying'), owid_product, 'per kilogram'),
                'supply_chain_breakdown': self._get_supply_chain_breakdown(datasets.get('supply_chain'), owid_product)
            }
    ...
    def _get_supply_chain_breakdown(self, df: Optional[pd.DataFrame], product: str) -> Dict[str, float]:
