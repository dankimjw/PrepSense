    ...
    def _estimate_nutrition_from_ingredients(self, ingredients: list[str]) -> dict[str, float]:
        ...
        for ingredient in ingredients:
            ...
            if any(
                ...
                for protein in ["chicken", "beef", "fish", "tofu", "beans"]
            ):
                nutrition["protein"] += 15.0

            if any(
                fiber in ingredient_lower
                for fiber in ["beans", "lentils", "vegetables", "broccoli"]
            ):
                nutrition["fiber"] += 5.0

            if any(
                vit_c in ingredient_lower for vit_c in ["citrus", "berries", "pepper", "tomato"]
            ):
...
class FoodNutritionLookupTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            return {"food_name": food_name, "error": str(e), "data_source": "error"}
...
class NutrientGapAnalysisTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            if target_nutrients:
                filtered_gaps = [
                    gap for gap in gap_analysis.gaps if gap.nutrient in target_nutrients
                ]
            else:
...
class RecipeNutritionEnhancerTool(BaseTool):
    ...
    def _run(self, recipe: dict[str, Any], detailed_lookup: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            enhanced_recipe["nutrition"] = (
                total_nutrition.to_dict()
                if hasattr(total_nutrition, "to_dict")
                else total_nutrition
            )
            ...
            logger.info(
                f"Enhanced recipe with nutrition data (source: {'usda' if detailed_lookup else 'estimated'})"
            )
            return enhanced_recipe
