...
class FoodDatabaseService:
    ...
    def _map_spoonacular_category(self, spoon_category: str, item_name: str) -> str:
        ...
        category_mapping = {
            'produce': 'produce_countable' if any(word in item_name.lower() for word in ['apple', 'banana', 'onion']) else 'produce_measurable',
            'dairy': 'dairy',
            'meat': 'meat_seafood',
    ...
    def _map_usda_category(self, usda_category: str, item_name: str) -> str:
        ...
        if 'vegetable' in usda_category or 'fruit' in usda_category:
            return 'produce_countable' if any(word in item_name.lower() for word in ['apple', 'banana', 'onion']) else 'produce_measurable'
    ...
    async def validate_unit_for_item(self, item_name: str, unit: str) -> Dict[str, Any]:
        ...
        if not is_valid or is_forbidden:
            ...
            if 'bar' in item_name.lower() and unit_lower in ['cup', 'ml', 'l', 'fl oz']:
                result['error'] = f"Bars cannot be measured in liquid units like '{unit}'. Try 'each' or 'package'."
    ...
    async def record_user_correction(self, item_name: str, old_category: str, new_category: str, user_id: int):
        ...
        try:
            ...
            VALUES (%s, %s, %s, %s, %s)
            """
            self.db_service.execute_query(query, (item_name, old_category, new_category, user_id, datetime.now()))
