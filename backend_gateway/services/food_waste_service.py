...
class FoodWasteService:
    ...
    def download_flw_data(self, csv_path: Optional[str] = None) -> pd.DataFrame:
        ...
        try:
            ...
            loss_stats = grouped['loss_percentage'].agg([
                'median', 'mean', 'min', 'max', 'count'
            ]).reset_index()
            
            loss_stats.columns = ['cpc_code', 'commodity_name', 'stage', 
                                 'median_loss_pct', 'mean_loss_pct', 
                                 'min_loss_pct', 'max_loss_pct', 'observations']
    ...
    def _get_default_loss_rate(self, food_name: str) -> float:
        ...
        if any(item in food_lower for item in ['apple', 'banana', 'berry', 'fruit', 'vegetable', 
        ...
        if include_supply_chain:
            ...
            if price_per_kg:
                ...
                impact['total_economic_impact'] = round(
                    impact['economic_loss'] + (impact['upstream_waste_kg'] * price_per_kg * supply_chain_cost_factor), 
                    2
                )
