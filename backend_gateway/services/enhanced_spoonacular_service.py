...
class EnhancedSpoonacularService(SpoonacularService):
    ...
    def __init__(self, db_service=None, enable_tracking: bool = True, enable_deduplication: bool = True):
        """
    ...
    async def search_recipes_by_ingredients(
        ...
        apply_dedup = self.enable_deduplication if enable_deduplication is None else enable_deduplication
        ...
        if self.enable_tracking and self.api_tracker:
            with self.api_tracker.track_api_call(
                ...
                if apply_dedup and self.deduplicator:
                    ...
                    fingerprints = [
                        self.deduplicator.generate_recipe_fingerprint(recipe) 
                    ...
                    if len(duplicate_ids) > 0:
                        ...
                        logger.info(f"🔍 Deduplication: {original_count} → {len(deduplicated_recipes)} recipes "
            ...
            if apply_dedup and self.deduplicator:
                deduplicated_recipes, duplicate_ids = self.deduplicator.deduplicate_recipes(recipes)
                if len(duplicate_ids) > 0:
                    logger.info(f"🔍 Deduplication (no tracking): {len(recipes)} → {len(deduplicated_recipes)} recipes")
                recipes = deduplicated_recipes
            ...
            with self.api_tracker.track_api_call(
                ...
                if self.enable_deduplication and self.deduplicator and recipe:
                    ...
                    context.set_deduplication_info(
                        ...
                        recipe_fingerprints=[fingerprint]
                    )
        ...
        if self.enable_tracking and self.api_tracker:
            with self.api_tracker.track_api_call(
                ...
                if apply_dedup and self.deduplicator and recipes:
                    ...
                    if len(duplicate_ids) > 0:
                        reduction_percent = (len(duplicate_ids) / original_count) * 100
                        logger.info(f"🔍 Complex search deduplication: {original_count} → {len(deduplicated_recipes)} recipes "
                                  f"({reduction_percent:.1f}% reduction)")
                
                # Update tracking context
                context.set_response_data(
                    response_status=200,
                    recipe_count=len(result.get('results', []))
                )
            ...
            if apply_dedup and self.deduplicator:
                ...
                if recipes:
                    ...
                    result['totalResults'] = len(deduplicated_recipes)
                    if len(duplicate_ids) > 0:
                        logger.info(f"🔍 Complex search deduplication (no tracking): {len(recipes)} → {len(deduplicated_recipes)} recipes")
        ...
        return {
            ...
            'features': {
                'api_tracking': {
                    ...
                    'status': '🟢 WORKING' if self.enable_tracking and self.api_tracker else '🔴 DISABLED'
                },
                'recipe_deduplication': {
                    'enabled': self.enable_deduplication,
                    'status': '🟢 WORKING' if self.enable_deduplication and self.deduplicator else '🔴 DISABLED'
                },
