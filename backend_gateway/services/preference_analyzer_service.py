...
class PreferenceAnalyzerService:
    ...
    def _analyze_cooking_frequency(self, cooking_history: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        frequency["most_cooked_recipes"] = [
            {"name": name, "count": count} for name, count in sorted_recipes[:5]
        ]
    ...
    def _extract_preference_insights(
        ...
        sorted_ingredients = sorted(
            patterns["common_ingredients"].items(), key=lambda x: x[1], reverse=True
        )

        # Take ingredients that appear frequently
        insights["favorite_ingredients"] = [
            ing for ing, count in sorted_ingredients[:10] if count >= max(3, total_liked * 0.25)
        ]
        ...
        sorted_categories = sorted(
            patterns["ingredient_categories"].items(), key=lambda x: x[1], reverse=True
        )
        insights["ingredient_categories_preference"] = [cat for cat, _ in sorted_categories[:5]]
    ...
    def _clean_ingredient_name(self, ingredient: str) -> str:
        ...
        cleaned = re.sub(
            r"\b(cup|cups|tsp|tbsp|tablespoon|teaspoon|oz|lb|g|kg|ml|l)\b", "", cleaned
        )
