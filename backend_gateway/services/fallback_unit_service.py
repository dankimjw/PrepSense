...
FOOD_ITEM_PATTERNS = {
    # Fruits
    'apple': 'each', 'banana': 'each', 'orange': 'each',
    'strawberry': 'cup', 'blueberry': 'cup', 'raspberry': 'cup',
    'grape': 'cup', 'cherry': 'cup', 'avocado': 'each',
    
    # Vegetables
    'potato': 'each', 'onion': 'each', 'carrot': 'each',
    'tomato': 'each', 'bell pepper': 'each', 'pepper': 'each',
    'spinach': 'cup', 'lettuce': 'cup', 'broccoli': 'cup',
    'cauliflower': 'cup',
    
    # Proteins
    'chicken': 'oz', 'beef': 'oz', 'pork': 'oz',
    'fish': 'oz', 'salmon': 'oz', 'tuna': 'oz',
    'egg': 'each', 'tofu': 'oz',
    
    # Dairy
    'milk': 'cup', 'cheese': 'oz', 'yogurt': 'cup', 'butter': 'tbsp',
    
    # Grains
    'rice': 'cup', 'pasta': 'cup', 'bread': 'slice',
    'cereal': 'cup', 'oats': 'cup',
    
    # Condiments
    'ketchup': 'tbsp', 'mustard': 'tbsp', 'mayo': 'tbsp',
    'oil': 'tbsp', 'vinegar': 'tbsp', 'salt': 'tsp', 'pepper': 'tsp'
}
...
class FallbackUnitService:
    ...
    def get_fallback_unit(self, item_name: str, category: Optional[str] = None) -> str:
        ...
        for pattern, unit in FOOD_ITEM_PATTERNS.items():
            if pattern in normalized_name:
                logger.info(f"Found specific pattern '{pattern}' for '{item_name}', using unit: {unit}")
                return unit
        ...
        if category and category in FOOD_CATEGORY_FALLBACKS:
            ...
            for pattern_key, pattern_unit in category_info['patterns'].items():
                if self._matches_subcategory(normalized_name, pattern_key):
                    logger.info(f"Found subcategory pattern '{pattern_key}' for '{item_name}', using unit: {pattern_unit}")
                    return pattern_unit
            
            # Fall back to category default
            default_unit = category_info['default_unit']
            logger.info(f"Using category default unit for '{item_name}' in '{category}': {default_unit}")
            return default_unit
    ...
    def validate_and_fix_quantity_unit(self, item_name: str, quantity: float, unit: str, 
                                     category: Optional[str] = None) -> Tuple[float, str]:
        """
        ...
        if self.is_likely_price(quantity, unit, item_name):
            logger.warning(f"Detected price-like data for '{item_name}': {quantity} {unit}, applying fallback")
    ...
    def _is_spice(self, item_name: str) -> bool:
        ...
        spice_patterns = ['salt', 'pepper', 'cinnamon', 'paprika', 'cumin', 'oregano', 'basil', 'thyme']
