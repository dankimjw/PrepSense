...
class EmbeddingService:
    ...
    async def generate_recipe_embedding(self, recipe: dict[str, Any]) -> list[float]:
        ...
        result = await self.generate_embedding(
            full_text, metadata={"type": "recipe", "id": recipe.get("id")}
        )
    ...
    async def generate_product_embedding(self, product: dict[str, Any]) -> list[float]:
        ...
        # Generate embedding
        result = await self.generate_embedding(
            full_text, metadata={"type": "product", "id": product.get("id")}
        )
        return result.embedding
    ...
    async def generate_pantry_item_embedding(self, item: dict[str, Any]) -> list[float]:
        ...
        # Generate embedding
        result = await self.generate_embedding(
            full_text, metadata={"type": "pantry_item", "id": item.get("id")}
        )
        return result.embedding
    ...
    async def generate_query_embedding(
        ...
        # Generate embedding
        result = await self.generate_embedding(
            full_text, metadata={"type": "query", "query_type": query_type}
        )
        return result.embedding
