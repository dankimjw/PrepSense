...
class RecipeImageManager:
    ...
    async def download_and_store_spoonacular_image(
        self, recipe_id: str, recipe_title: str, spoonacular_url: str
    ) -> Optional[str]:
        ...
        try:
            ...
            # Update database
            await self.upsert_image_record(
                recipe_id, recipe_title, signed_url, filename, local_path, expiration_date
            )
    ...
    async def generate_and_store_recipe_image(
        ...
        try:
            ...
            expires_at = datetime.now(timezone.utc) + timedelta(days=7)
            await self.upsert_image_record(
                recipe_id, recipe_title, gcs_signed_url, gcs_blob_path, local_path, expires_at
            )
    ...
    async def get_image_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            local_files = (
                list(self.local_images_dir.glob("*.png")) if self.local_images_dir.exists() else []
            )
