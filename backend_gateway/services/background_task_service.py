...
class BackgroundTaskService:
    ...
    async def on_preferences_updated(
        self, user_id: int, trigger_reason: str = "preferences_update"
    ):
        """Handle preferences update event"""
        try:
            logger.info(
                f"Preferences updated for user {user_id}, triggering preference cache refresh"
            )
    ...
    async def on_recipe_interaction(
        self, user_id: int, recipe_id: str, action: str, metadata: Optional[dict[str, Any]] = None
    ):
    ...
    async def warm_cache_for_user(self, user_id: int, reason: str = "manual"):
        ...
        try:
            ...
            # Ensure fresh cache
            await self.cache_manager.ensure_fresh_cache(
                user_id, self.db_service, force_refresh=True
            )
...
async def handle_recipe_interaction(
    user_id: int, recipe_id: str, action: str, metadata: Optional[dict[str, Any]] = None
):
    """Handle recipe interaction event"""
    service = get_background_task_service()
