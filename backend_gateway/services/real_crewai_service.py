...
class RealCrewAIService:
    ...
    async def process_message(self, user_id: int, message: str, use_preferences: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            preference_artifact = self.cache_manager.get_preference_artifact(user_id) if use_preferences else None
    ...
    async def _get_recommendations_fallback(self, crew_input: CrewInput) -> CrewOutput:
        ...
        ranked_recipes = self._rank_recipes_intelligently(
            crew_input.recipe_candidates,
            crew_input.pantry_artifact,
            crew_input.preference_artifact
        )
        
        # Generate contextual response
        response_text = self._generate_contextual_response(
            crew_input.user_message,
            ranked_recipes,
            crew_input.pantry_artifact
        )
    ...
    def _rank_recipes_intelligently(self, recipes: List[Dict], pantry_artifact, preference_artifact) -> List[Dict]:
        ...
        for recipe in recipes:
            ...
            if pantry_artifact:
                available_ingredients = {item['name'].lower() for item in pantry_artifact.normalized_items}
                ...
                if recipe_ingredients:
                    ...
                    match_ratio = matches / total_recipe_ingredients if total_recipe_ingredients > 0 else 0
                    score += match_ratio * 0.4
                
                # Bonus for using expiring items
                if pantry_artifact.expiry_analysis:
                    expiring_items = {item['name'].lower() for item in pantry_artifact.expiry_analysis.get('expiring_soon', [])}
            ...
            if preference_artifact:
                ...
                if 'vegetarian' in preference_artifact.dietary_restrictions and not recipe.get('vegetarian', False):
                    score -= 0.5
                if 'vegan' in preference_artifact.dietary_restrictions and not recipe.get('vegan', False):
                    score -= 0.5
    ...
    def _generate_contextual_response(self, message: str, recipes: List[Dict], pantry_artifact) -> str:
        ...
        # Add recipe count
        if recipes:
            base_response += f". I found {len(recipes)} recipe{'s' if len(recipes) > 1 else ''} for you!"
        else:
    ...
    def _format_chat_response(self, crew_output: CrewOutput, crew_input: CrewInput) -> Dict[str, Any]:
