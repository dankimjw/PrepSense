...
class HybridChatRecipeService:
    ...
    def generate_single_recipe(
        ...
        try:
            ...
            if source != RecipeSource.MOCK_ENHANCED:
                dietary_restrictions = user_preferences.get("dietary_preferences", []) if user_preferences else []
    ...
    def get_recipe_suggestions(
        ...
        try:
            # Check mock mode
            if is_chat_recipes_mock_enabled():
                mock_recipes = get_mock_recipes_for_ingredients(available_ingredients, max_suggestions)
        ...
        try:
            ...
            frontend_compatibility = {
                "has_spoonacular_format": all(
                    field in recipe for field in ["id", "title", "extendedIngredients", "analyzedInstructions"]
                ),
                "has_chat_format": all(
                    field in recipe for field in ["name", "ingredients", "instructions"]
                ),
                "has_nutrition_data": "nutrition" in recipe and "nutrients" in recipe.get("nutrition", {}),
                "has_caloric_breakdown": "nutrition" in recipe and "caloricBreakdown" in recipe.get("nutrition", {}),
            ...
            return {
                ...
                "recommendation": "Ready for frontend" if overall_compatibility else "Needs enhancement"
            }
    ...
    def enhance_existing_recipe(self, recipe: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for safety_field in ["safety_status", "safety_violations", "safety_warnings", "allergen_risks"]:
                if safety_field not in enhanced_recipe:
                    enhanced_recipe[safety_field] = [] if "violations" in safety_field or "warnings" in safety_field or "risks" in safety_field else "SAFE"
    ...
    def _get_mock_recipes(self, available_ingredients: List[str], max_recipes: int) -> List[Dict[str, Any]]:
    ...
    def _generate_openai_recipes(
        ...
        try:
            ...
            for i, recipe_name in enumerate(recipe_names[:max_recipes]):
                recipe = self.openai_service.generate_enhanced_recipe(
                    recipe_name=recipe_name,
                    available_ingredients=available_ingredients,
    ...
    def _group_ingredients(self, ingredients: List[str]) -> Dict[str, List[str]]:
        ...
        for ingredient in ingredients:
            ...
            if any(protein in ingredient_lower for protein in ["chicken", "beef", "pork", "fish", "tofu", "eggs"]):
                groups["protein"].append(ingredient)
            elif any(veg in ingredient_lower for veg in ["tomato", "onion", "carrot", "pepper", "broccoli", "spinach"]):
                groups["vegetables"].append(ingredient)
            elif any(grain in ingredient_lower for grain in ["rice", "pasta", "bread", "flour", "quinoa"]):
            ...
            elif any(spice in ingredient_lower for spice in ["salt", "pepper", "garlic", "oregano", "basil"]):
                groups["spices"].append(ingredient)
        
        return groups
    
    def _get_fallback_recipes(self, available_ingredients: List[str], max_recipes: int) -> List[Dict[str, Any]]:
    ...
    def _create_emergency_fallback(self, recipe_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "occasions": ["casual"],
            "extendedIngredients": [],
            "analyzedInstructions": [{"steps": [{"number": 1, "step": "Combine ingredients and cook as desired."}]}],
