...
class FoodLookupTool:
    ...
    def estimate_nutrition_from_ingredients(self, ingredients: List[str]) -> NutrientProfile:
        ...
        for ingredient in ingredients:
            try:
                ...
                logger.warning(f"Could not estimate nutrition for ingredient: {ingredient}, error: {e}")
                continue
    ...
    def _parse_ingredient(self, ingredient: str) -> Tuple[str, float]:
        ...
        match = re.match(r'^(\d+(?:\.\d+)?)\s*(?:cups?|tbsp|tsp|oz|lbs?|g|kg|ml|l)?\s*(.+)', ingredient.strip())
        ...
        food_name = re.sub(r'\b(chopped|diced|sliced|minced|fresh|frozen|dried|cooked)\b', '', food_name)
