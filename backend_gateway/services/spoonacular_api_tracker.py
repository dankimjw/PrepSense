...
class SpoonacularAPITracker:
        ...
        try:
            ...
            if self.db_service:
                try:
                    ...
                    if result:
                        call_id = result[0]['id']
                        logger.debug(f"Logged API call {call_id}: {endpoint} ({response_status}) - "
        ...
        try:
            ...
            if result:
                ...
                return {
                    ...
                    'duplicate_detection_rate': (duplicate_detections / total_calls) if total_calls > 0 else 0,
    ...
    def _calculate_in_memory_stats(self, user_id: Optional[int], days: int) -> Dict[str, Any]:
        ...
        relevant_calls = [
            call for call in self._in_memory_calls
        ...
        return {
            ...
            'avg_response_time_ms': sum(call.get('response_time_ms', 0) for call in relevant_calls) / total_calls,
