...
class SmartUnitValidator:
    ...
    def __init__(self, db_pool: asyncpg.Pool):
        ...
        self.category_unit_rules = {
            ...
            'Produce': {
                'fruits': {
                    'allowed': ['lb', 'oz', 'kg', 'g', 'each', 'pint', 'quart', 'basket', 'bag', 'container'],
            ...
            'Dairy and Egg Products': {
                'liquid_dairy': {
                    'allowed': ['ml', 'l', 'fl oz', 'cup', 'pint', 'quart', 'gallon', 'bottle', 'carton'],
            ...
            'Beverages': {
                'default': {
                    'allowed': ['ml', 'l', 'fl oz', 'cup', 'pint', 'quart', 'gallon', 'bottle', 'can', 'container'],
    ...
    async def validate_and_suggest_unit(
        ...
        # Suggest better unit
        suggested_unit = self._suggest_best_unit(
            clean_item, 
            clean_unit, 
            unit_rules, 
            quantity
        ...
        # Add explanation
        if is_forbidden:
            result['reason'] = f"{item_name} cannot be measured in {clean_unit}. {unit_rules.get('examples', '')}"
            result['severity'] = 'error'
        elif not is_valid:
            result['reason'] = f"Unusual unit for {item_name}. Consider: {', '.join(unit_rules.get('preferred', []))}"
                ...
                if not result['is_valid'] and result['suggested_unit']:
                    if auto_fix:
                        ...
                        await conn.execute("""
                            ...
                            SET quantity_unit = $1
                            WHERE id = $2
                        """, result['suggested_unit'], result['pantry_item_id'])
