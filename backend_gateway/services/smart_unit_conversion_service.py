...
class SmartUnitConversionService:
    ...
    def convert(
        ...
        if not item_name:
            return (
                0.0,
                "error",
                f"Cannot convert {from_unit} to {to_unit} without knowing the item",
            )
    ...
    def _has_density_data(
        self, item_name: str, from_category: UnitCategory, to_category: UnitCategory
    ) -> bool:
