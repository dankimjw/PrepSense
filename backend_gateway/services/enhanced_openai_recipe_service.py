...
class EnhancedOpenAIRecipeService:
    ...
    def _generate_structured_recipe(
        ...
        prompt = self._create_recipe_prompt(
            recipe_name, available_ingredients, dietary_restrictions, 
            allergens, cuisine_type, cooking_time, servings
        )
        ...
        try:
            ...
            if recipe_json:
                ...
                logger.warning("Failed to extract JSON from OpenAI response, creating structured fallback")
...
"""
    ...
    def _add_recipe_metadata(self, recipe: Dict[str, Any], available_ingredients: List[str]) -> Dict[str, Any]:
        """Add metadata and compatibility fields"""
        # Generate unique ID
        ...
        recipe["match_score"] = len(matched_ingredients) / len(recipe_ingredients) if recipe_ingredients else 0
    ...
    def _create_structured_fallback(self, recipe_name: str, available_ingredients: List[str]) -> Dict[str, Any]:
    ...
    def _create_fallback_recipe(self, recipe_name: str, available_ingredients: List[str]) -> Dict[str, Any]:
    ...
    def batch_generate_recipes(
        self,
        recipe_requests: List[Dict[str, Any]],
        ...
        for i in range(0, len(recipe_requests), max_concurrent):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_concurrent) as executor:
                futures = [
                    executor.submit(self.generate_enhanced_recipe, **request)
    ...
    def validate_recipe_structure(self, recipe: Dict[str, Any]) -> Dict[str, Any]:
        ...
        required_fields = ["id", "title", "extendedIngredients", "analyzedInstructions", "nutrition"]
