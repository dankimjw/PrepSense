...
class PantryItemManager:
    ...
    def add_items_batch(self, user_id: int, items: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        random_component = random.randint(0, 9999)
        base_id = (
            timestamp_ms * 10000 + random_component
        ) % 1000000000  # Keep it under BigQuery INT64 max
        ...
        for idx, item in enumerate(items):
            try:
                ...
                logger.info(
                    f"Pantry item insert took {(datetime.now() - insert_start).total_seconds()}s"
                )
                ...
                logger.info(
                    f"Product insert took {(datetime.now() - product_start).total_seconds()}s"
                )
                ...
                logger.info(
                    f"Saved item: {item.get('item_name')} (pantry_item_id: {pantry_item_id})"
                )

            except Exception as e:
                logger.error(f"Error saving item {item.get('item_name')}: {str(e)}")
                errors.append({"item_name": item.get("item_name"), "error": str(e)})
    ...
    def delete_recent_items(self, user_id: int, hours: Optional[float] = None) -> dict[str, Any]:
        ...
        try:
            ...
            if hours is not None:
                time_filter = (
                    f"AND pi.created_at >= CURRENT_TIMESTAMP - INTERVAL '{int(hours * 60)} minutes'"
                )
            ...
            items_to_delete = self.db_service.execute_query(
                select_query, {"pantry_ids": pantry_ids}
            )
            ...
            product_ids = [
                item["product_id"] for item in items_to_delete if item["product_id"] is not None
            ]
            if product_ids:
    ...
    def delete_item(self, user_id: int, pantry_item_id: int) -> dict[str, Any]:
        ...
        result = self.db_service.execute_query(
            verify_query, {"user_id": user_id, "pantry_item_id": pantry_item_id}
        )

        if not result:
            return {"deleted": False, "message": "Item not found or doesn't belong to user"}
