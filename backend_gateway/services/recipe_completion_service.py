...
from backend_gateway.constants.units import convert_quantity, normalize_unit, get_unit_category, UnitCategory
...
class RecipeCompletionService:
    ...
    def match_ingredient_to_pantry(
        ...
        for item in pantry_items:
            ...
            elif (ingredient_lower.rstrip('s') == item_name_lower.rstrip('s') or
                  ingredient_lower.rstrip('es') == item_name_lower.rstrip('es')):
                match_score = 90
    ...
    def calculate_quantity_to_use(
        ...
        converted_quantity = convert_quantity(needed_quantity, needed_unit_norm, available_unit_norm)
        
        if converted_quantity is not None:
            metadata['conversion_performed'] = True
            metadata['converted_quantity'] = converted_quantity
            metadata['conversion_details'] = f"{needed_quantity} {needed_unit} = {converted_quantity:.2f} {available_unit}"
        ...
        if needed_category != available_category:
            ...
            if needed_category == UnitCategory.COUNT and available_category in [UnitCategory.WEIGHT, UnitCategory.VOLUME]:
                # e.g., "2 tomatoes" vs "500g tomatoes"
                # This would need ingredient-specific conversion tables
                metadata['warnings'].append("Count to weight/volume conversion requires ingredient-specific data")
                return None, available_unit_norm, metadata
        
        # Same category but no conversion available
        metadata['warnings'].append(f"No conversion available between {needed_unit} and {available_unit}")
        return None, available_unit_norm, metadata
    ...
    def process_ingredient_consumption(
        ...
        for pantry_item in matching_items:
            ...
            quantity_to_use, unit_used, conversion_meta = RecipeCompletionService.calculate_quantity_to_use(
                remaining_needed,
                remaining_needed_unit,
                current_quantity,
                pantry_unit
            )
            ...
            try:
                ...
                update_params = {
                    ...
                    "new_used_quantity": (pantry_item.get('used_quantity', 0) or 0) + quantity_to_use,
                ...
                logger.error(f"Error updating pantry item {pantry_item['pantry_item_id']}: {str(e)}")
                result['warnings'].append(f"Failed to update {pantry_item['product_name']}: {str(e)}")
