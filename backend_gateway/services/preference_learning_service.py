...
class PreferenceLearningService:
    ...
    async def suggest_new_preferences(self, user_id: int) -> List[Dict]:
        ...
        top_ingredients = [
            ing for ing, count in ingredient_counts.most_common(20)
        ...
        if avg_cook_time:
            time_pref = 'quick' if avg_cook_time < 25 else 'medium' if avg_cook_time < 45 else 'leisurely'
            suggestions.append({
    ...
    async def _get_negative_interactions(self, user_id: int) -> List[Dict]:
        ...
        return await self._get_user_history(
            user_id, 
            days=90,
            actions=['dismissed', 'rated_negative', 'removed']
        )
    ...
    async def _analyze_ingredient_frequency(
        self, 
        user_id: int, 
        history: List[Dict]
    ) -> Counter:
    ...
    async def _analyze_cuisine_frequency(
        self, 
        user_id: int, 
        history: List[Dict]
    ) -> Counter:
    ...
    async def _detect_dietary_patterns(self, history: List[Dict]) -> Dict[str, float]:
        ...
        for pattern in patterns:
            matching = sum(
                1 for item in history
                if item.get('recipe_data', {}).get('dietary_tags') and
                pattern.replace('_', '-') in item['recipe_data']['dietary_tags']
            )
    ...
    def _normalize_ingredient(self, ingredient: str) -> Optional[str]:
        ...
        ingredient_clean = re.sub(
            r'\b\d+\.?\d*\s*(cup|tbsp|tsp|oz|lb|g|kg|ml|l)s?\b', 
            '', 
            ingredient_lower
        )
        ...
        remove_words = [
            'fresh', 'frozen', 'canned', 'dried', 'chopped', 
            'diced', 'sliced', 'minced', 'ground'
        ]
