...
from backend_gateway.routers.backup_recipes_router import search_backup_recipes, get_backup_recipe_details
...
class RecipeFallbackService:
    ...
    async def search_recipes(
        ...
        try:
            ...
            if config.enable_backup_recipes:
                ...
                high_quality_backup = [r for r in backup_results if r.match_ratio and r.match_ratio > 0.7]
            ...
            if (config.enable_spoonacular and 
                len(all_results) < config.total_max_results):
            ...
            if (config.enable_openai and 
                len(all_results) < config.total_max_results // 2):
            ...
            logger.info(f"🎉 Recipe search completed in {elapsed_time:.2f}s, returning {len(sorted_results)} results")
        ...
        try:
            ...
            for recipe_data in spoonacular_recipes:
                result = RecipeResult(
                    ...
                    image_url=recipe_data.get('image'),
                    ready_in_minutes=recipe_data.get('readyInMinutes'),
                    used_ingredients=[ing.get('name', '') for ing in recipe_data.get('usedIngredients', [])],
                    missed_ingredients=[ing.get('name', '') for ing in recipe_data.get('missedIngredients', [])],
                    match_ratio=recipe_data.get('usedIngredientCount', 0) / max(1, len(ingredients or [])),
                    raw_data=recipe_data
                )
                results.append(result)
    ...
    def _update_performance_stats(self, elapsed_time: float, results: List[RecipeResult]):
        ...
        if total_queries > 0:
            ...
            self._performance_stats['backup_success_rate'] = backup_success / max(1, self._performance_stats['backup_queries'])
            self._performance_stats['spoonacular_success_rate'] = spoonacular_success / max(1, self._performance_stats['spoonacular_queries'])
