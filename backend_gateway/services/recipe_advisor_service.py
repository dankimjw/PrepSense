...
class CrewAIService:
        ...
        try:
            ...
            for recipe in recipes:
                ...
                standardized = {
                    ...
                    'match_score': (len(used_ingredients) / (len(used_ingredients) + missing_count)) if (len(used_ingredients) + missing_count) > 0 else 0,
    ...
    def _extract_ingredients(self, recipe: Dict[str, Any]) -> List[str]:
        ...
        if 'extendedIngredients' in recipe:
            return [ing.get('original', ing.get('name', '')) for ing in recipe['extendedIngredients']]
    ...
    def _is_allergen_in_ingredient(self, allergen: str, ingredient: str) -> bool:
        ...
        allergen_keywords = {
            'nuts': ['nuts', 'peanut', 'almond', 'walnut', 'pecan', 'cashew', 'pistachio', 'hazelnut'],
    ...
    def _format_response(self, recipes: List[Dict[str, Any]], pantry_items: List[Dict[str, Any]], message: str, user_preferences: Dict[str, Any]) -> str:
        ...
        if demo_recipes:
            response_parts.append(f"🌟 Great! I found {len(demo_recipes)} featured high-protein recipe{'s' if len(demo_recipes) > 1 else ''} that would be perfect for you!")

        # Context-aware opening
        if 'quick' in message.lower() or 'fast' in message.lower():
            quick_recipes = [r for r in recipes[:3] if r.get('time', 999) <= 30]
            if quick_recipes:
                response_parts.append(f"I found {len(quick_recipes)} quick recipe{'s' if len(quick_recipes) > 1 else ''} (30 min or less) using your pantry items.")
        elif 'healthy' in message.lower():
            response_parts.append("I've focused on nutritionally balanced recipes with good protein content.")
        elif 'dinner' in message.lower():
            response_parts.append("Here are some great dinner options based on what you have:")
        else:
            response_parts.append(f"Based on your pantry items, I found {len(recipes)} recipe{'s' if len(recipes) > 1 else ''} you can make!")

        # Ingredient availability info
        can_make_now = [r for r in recipes[:3] if r.get('can_make', False)]
        if can_make_now:
            response_parts.append(f"🎉 {len(can_make_now)} of these recipes can be made with ingredients you already have!")

        # Expiring items priority
        expiring_recipes = [r for r in recipes[:3] if r.get('evaluation', {}).get('uses_expiring', False)]
        if expiring_recipes:
            response_parts.append(f"💡 {len(expiring_recipes)} recipe{'s' if len(expiring_recipes) > 1 else ''} will help use up ingredients expiring soon.")
