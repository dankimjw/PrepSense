...
AGENT_QUEUE_DEPTH = Gauge(
    "crewai_agent_queue_depth",
    "Current queue depth for agent executions",
    ["agent_type"]
)
...
class AgentExecutionContext:
    ...
    def start(self):
        ...
        logger.info(
            f"CrewAI agent execution started",
            extra={
                "agent_type": self.agent_type.value,
                "task_type": self.task_type,
                "task_id": self.task_id,
    ...
    def add_decision_point(self, decision: str, reasoning: str, confidence: float = 1.0, context: Optional[Dict[str, Any]] = None):
    ...
    def record_tool_usage(self, tool_name: str, duration_seconds: float, success: bool, inputs: Optional[Dict[str, Any]] = None, outputs: Optional[Dict[str, Any]] = None):
    ...
    def _record_completion(self):
        ...
        logger.log(
            log_level,
            f"CrewAI agent execution completed",
            extra={
                "agent_type": self.agent_type.value,
                "task_type": self.task_type,
                "task_id": self.task_id,
...
class CrewAIObservabilityManager:
    ...
    def create_execution_context(
        ...
        if enable_circuit_breaker and not circuit_breaker.can_execute():
            raise RuntimeError(f"Agent {agent_type.value} is temporarily unavailable due to circuit breaker (state: {circuit_breaker.state})")
    ...
    def complete_execution_context(self, context: AgentExecutionContext):
        ...
        with self._lock:
            ...
            AGENT_CIRCUIT_BREAKER_STATE.labels(
                agent_type=context.agent_type.value
            ).set(circuit_breaker.get_state_value())
    ...
    def get_agent_health_detailed(self, agent_type: AgentType) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_executions = [
                h for h in self.execution_history[-100:]  # Last 100 executions
                if h.get("agent_type") == agent_type.value
            ]
            
            # Calculate statistics
            total_executions = len(recent_executions)
            successful_executions = len([h for h in recent_executions if h.get("status") == "success"])
        ...
        for agent in AgentType:
            ...
            if agent_history:
                ...
                agent_stats[agent.value] = {
                    ...
                    "average_duration": sum(agent_durations) / len(agent_durations) if agent_durations else 0.0,
                    "total_tokens": sum(
    ...
    def cleanup_completed_executions(self):
        ...
        completed_ids = [
            task_id for task_id, context in self.active_executions.items()
                ...
                with context:
                    ...
                    for attempt in range(context.retry_config.max_retries + 1):
                        try:
                            ...
                            if any(term in error_msg for term in ["validation", "invalid", "authentication", "unauthorized"]):
                                logger.error(
                                    f"Non-retryable error in agent execution",
                                    extra={
                                        "agent_type": agent_type.value,
                                        "task_type": task_type,
                                        "task_id": context.task_id,
...
def get_agent_health() -> Dict[str, Any]:
    ...
    for agent_type in AgentType:
        ...
        if agent_health["health_status"] == "unhealthy":
            ...
            system_issues.extend([f"{agent_type.value}: {warning}" for warning in agent_health["warnings"]])
...
def get_agent_performance_report() -> Dict[str, Any]:
    ...
    for agent_type in AgentType:
        ...
        recent_executions = [
            h for h in observability_manager.execution_history[-200:]
            if h.get("agent_type") == agent_type.value
        ]
        ...
        if len(recent_executions) >= 10:
            ...
            older_success_rate = len([h for h in older_half if h.get("status") == "success"]) / len(older_half)
            newer_success_rate = len([h for h in newer_half if h.get("status") == "success"]) / len(newer_half)
            ...
            performance_trend = {
                "success_rate_trend": "improving" if newer_success_rate > older_success_rate else "declining",
                "performance_trend": "improving" if newer_avg_duration < older_avg_duration else "declining",
                "success_rate_change": newer_success_rate - older_success_rate,
                "duration_change_percent": ((newer_avg_duration - older_avg_duration) / older_avg_duration * 100) if older_avg_duration > 0 else 0
            }
        else:
            performance_trend = {
        ...
        performance_data[agent_type.value] = {
            ...
            "recent_activity": {
                "last_24h_executions": len([
                    h for h in recent_executions 
                    if h.get("timestamp") and (datetime.utcnow() - datetime.fromisoformat(h["timestamp"])).total_seconds() < 86400
                ])
            }
        }
...
def get_agent_debugging_info(agent_type: AgentType, task_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if task_id:
        ...
        # Get recent executions for this agent
        recent_executions = [
            h for h in observability_manager.execution_history[-50:]
            if h.get("agent_type") == agent_type.value
        ]
