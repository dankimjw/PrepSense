...
from .models import PantryArtifact, PantryState, PreferenceArtifact, PreferenceState
...
class PantryAnalysisFlow(Flow[PantryState]):
    ...
    def fetch_pantry_data(self):
        ...
        try:
            ...
            logger.info(
                f"Starting pantry analysis for user {user_id} - trigger: {self.args.trigger_reason}"
            )
    ...
    def compute_expiry_analysis(self, trigger_result):
        ...
        try:
            ...
            if total > 0:
                urgent_count = len(expiry_analysis["expired"]) + len(
                    expiry_analysis["expiring_soon"]
                )
                expiry_analysis["urgency_score"] = urgent_count / total

            self.state.expiry_analysis = expiry_analysis
            logger.info(
                f"Computed expiry analysis - {len(expiry_analysis['expiring_soon'])} items expiring soon"
            )
...
class PreferenceLearningFlow(Flow[PreferenceState]):
    ...
    def fetch_user_interactions(self):
        ...
        try:
            user_id = self.args.user_id
            logger.info(
                f"Starting preference learning for user {user_id} - trigger: {self.args.trigger_reason}"
            )
    ...
    def build_preference_vector(self, trigger_result):
        ...
        try:
            ...
            logger.info("Built preference vector and cuisine preferences: %s", cuisine_preferences)
            return "vector_built"

        except Exception as e:
            logger.error("Failed to build preference vector: %s", e)
    ...
    def persist_preference_artifact(self, trigger_result):
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Successfully persisted preference artifact for user {self.args.user_id}"
                )
                return "artifact_persisted"
            else:
...
class BackgroundFlowOrchestrator:
    ...
    async def warm_user_cache(
        self, user_id: int, trigger_reason: str = "manual"
    ) -> Dict[str, bool]:
        ...
        try:
            ...
            logger.info("Cache warming completed for user %s: %s", user_id, results)
            return results

        except Exception as e:
            logger.error("Failed to warm cache for user %s: %s", user_id, e)
    ...
    def should_warm_cache(self, user_id: int) -> bool:
        ...
        # Check if user has fresh pantry and preference artifacts
        try:
            ...
            # If either artifact is missing, cache needs warming
            if not pantry_artifact or not preference_artifact:
                return True
            
            # Check if artifacts are recent (less than 24 hours old)
            from datetime import datetime, timedelta
            now = datetime.now()
            
            # Check pantry artifact age
            if pantry_artifact.created_at:
                pantry_age = now - pantry_artifact.created_at
                if pantry_age > timedelta(hours=24):
                    return True
            
            # Check preference artifact age
            if preference_artifact.created_at:
                preference_age = now - preference_artifact.created_at
                if preference_age > timedelta(hours=24):
                    return True
            
            return False
        except Exception as e:
            logger.warning("Error checking cache freshness for user %s: %s", user_id, e)
            # If we can't check, assume cache needs warming
            return True
