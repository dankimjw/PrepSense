...
from ..models.nutrition import NutrientGapAnalysis, DailyIntakeLog, NutrientProfile
    ...
    def generate_priority_message(self, analysis: NutrientGapAnalysis) -> str:
        ...
        if significant_deficiencies:
            ...
            if len(deficiency_names) == 1:
                ...
                message_parts.append(f"You need {amounts[0]} more {deficiency_names[0]} and {amounts[1]} more {deficiency_names[1]}")
...
class NutrientAuditorFlow(Flow):
    ...
    def analyze_nutrient_gaps(self, _) -> str:
        ...
        try:
            ...
            logger.info(f"Analysis complete. Found {len(analysis.priority_deficiencies)} deficiencies, {len(analysis.priority_excesses)} excesses")
    ...
    def save_analysis_results(self, _) -> str:
        ...
        try:
            ...
            if self.state.gap_analysis:
                logger.info(f"Analysis saved for user {self.state.user_id} on {self.state.analysis_date}")
                return "saved"
            else:
                return "error"
