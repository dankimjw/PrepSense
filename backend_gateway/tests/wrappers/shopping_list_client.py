...
class MockShoppingListClient:
    ...
    def add_shopping_list_items(
        ...
        for item in items:
            ...
            existing_item = next(
                (i for i in self.shopping_lists[user_id] 
    ...
    def remove_shopping_list_items(
        self, 
        user_id: int, 
        item_names: List[str]
    ) -> Dict[str, Any]:
        ...
        self.shopping_lists[user_id] = [
            item for item in self.shopping_lists[user_id]
            if item['item_name'] not in item_names
        ]
...
class ShoppingListClientWrapper:
    ...
    def remove_shopping_list_items(
        ...
        if self._db_service:
            ...
            result = self._db_service.execute_query(
                query, 
                {"user_id": user_id, "item_names": item_names}
            )
    ...
    def update_shopping_list_item(
        ...
        if self._db_service:
            ...
            return {
                'success': success,
                'message': 'Item updated' if success else 'Item not found'
            }
