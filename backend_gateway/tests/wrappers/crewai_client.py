...
class MockCrewAIClient:
    ...
    def __init__(self):
        ...
        self.advisor_goal = "Recommend the best recipes based on pantry items, preferences, and context"
    ...
    def generate_advice(
        ...
        if expiring_count > 0 and 'expir' in message.lower():
            advice_parts.append(f"I found {expiring_count} items expiring soon and prioritized recipes using them.")
        ...
        return " ".join(advice_parts) if advice_parts else "Here are some recipe recommendations based on your pantry items."
    ...
    def process_message(
        ...
        evaluated_recipes.sort(
            key=lambda r: (r['evaluation']['uses_expiring'], r['evaluation']['nutritional_balance'] == 'good'),
            reverse=True
        )
    ...
    def _generate_mock_recipes(self, pantry_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i in range(5):
            ...
            recipe = {
                ...
                'cuisine_type': random.choice(['italian', 'asian', 'mexican', 'american', 'mediterranean']),
...
class CrewAIClientWrapper:
    ...
    def evaluate_recipe_fit(
        ...
        if self._service and hasattr(self._service.recipe_advisor, 'evaluate_recipe_fit'):
            return self._service.recipe_advisor.evaluate_recipe_fit(recipe, user_preferences, pantry_analysis)
