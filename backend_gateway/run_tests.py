...
def main():
    ...
    test_results["unit"] = run_command(
        [sys.executable, "-m", "pytest", "tests/domain/", "-v", "--tb=short", "-m", "not slow"],
        "Unit Tests",
    )
    ...
    test_results["integration"] = run_command(
        [sys.executable, "-m", "pytest", "tests/api/", "-v", "--tb=short"], "Integration Tests"
    )

    # 3. Database Tests
    print("\n🗄️ Running Database Tests")
    test_results["database"] = run_command(
        [sys.executable, "-m", "pytest", "tests/db/", "-v", "--tb=short"], "Database Tests"
    )

    # 4. Contract Tests
    print("\n📋 Running Contract Tests")
    test_results["contract"] = run_command(
        [sys.executable, "-m", "pytest", "tests/contracts/", "-v", "--tb=short"], "Contract Tests"
    )

    # 5. Performance Tests
    print("\n⚡ Running Performance Tests")
    test_results["performance"] = run_command(
        [sys.executable, "-m", "pytest", "tests/perf/", "-v", "--tb=short", "-m", "performance"],
        "Performance Tests",
    )
    ...
    test_results["ingredient_matching"] = run_command(
        [sys.executable, "test_ingredient_matching_simple.py"], "Ingredient Matching Tests"
    )
    ...
    test_results["black"] = run_command(
        ["python", "-m", "black", "--check", "--diff", "."], "Black Code Formatting"
    )

    # Ruff linter
    test_results["ruff"] = run_command(["python", "-m", "ruff", "check", "."], "Ruff Linting")
    ...
    test_results["bandit"] = run_command(
        ["python", "-m", "bandit", "-r", ".", "-f", "json"], "Bandit Security Scan"
    )

    # pip-audit
    test_results["pip_audit"] = run_command(
        ["python", "-m", "pip_audit", "--desc"], "Pip Audit Security Check"
    )
