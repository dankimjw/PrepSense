...
def fix_duplicate_pantry_items(dry_run=True):
    ...
    for item in items:
        ...
        product_key = f"{item['product_name']}|{item.get('brand_name', '')}|{item['unit_of_measurement']}"
        ...
        for product_key, items_list in products.items():
            if len(items_list) > 1:
                ...
                items_sorted = sorted(items_list, key=lambda x: (
                    x.get('expiration_date') or '9999-12-31',
                    x['created_at']
                ))
    ...
    if consolidations:
        ...
        for cons in consolidations[:10]:
            ...
            print(f"  Deleting IDs: {cons['delete_item_ids'][:5]}{'...' if len(cons['delete_item_ids']) > 5 else ''}")
    ...
    for cons in consolidations:
        try:
            ...
            db_service.execute_query(update_query, {
                'quantity': cons['new_quantity'],
                'item_id': cons['keep_item_id']
            })
            ...
            if cons['delete_item_ids']:
                ...
                db_service.execute_query(delete_query, {
                    'item_ids': cons['delete_item_ids']
                })
