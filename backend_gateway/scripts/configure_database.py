...
def update_env_file(use_postgres: bool):
    ...
    for line in lines:
        ...
        if not use_postgres and any(
            line.strip().startswith(prefix) for prefix in ["POSTGRES_", "CLOUD_SQL_"]
        ):
            continue
    ...
    if use_postgres:
        ...
        if not has_pg_config:
            ...
            new_lines.append(
                "POSTGRES_PASSWORD=your-secure-password  # Set via environment variable\n"
            )
            new_lines.append(
...
def main():
    ...
    if choice == "1":
        ...
        if update_env_file(True):
            ...
            print(
                "   gcloud sql instances describe prepsense-postgres --project=adsp-34002-on02-prep-sense"
            )
            print("\n2. Run migration if needed:")
            print(
                "   POSTGRES_PASSWORD=your-password python backend_gateway/scripts/migrate_bigquery_to_postgres.py"
            )
