...
class FoodDatabaseImportPipeline:
                            ...
                            async with session.get(search_url, params=params) as response:
                                if response.status == 200:
                                    ...
                                    for i in range(0, len(foods), self.batch_size):
                                        ...
                                        batch_results = await self._process_usda_batch(
                                            batch, session
                                        )
    ...
    async def _process_usda_batch(
        ...
        for food in foods:
            try:
                ...
                logger.error(
                    f"Error processing USDA food {food.get('description', 'unknown')}: {str(e)}"
                )
                results["failed"] += 1
                results["errors"].append(str(e))
    ...
    async def _import_openfoodfacts_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not export_file.exists() or datetime.fromtimestamp(
    ...
    async def _process_off_batch(self, products: list[dict]) -> dict[str, Any]:
        ...
        for product in products:
            try:
                ...
                logger.error(
                    f"Error processing OFF product {product.get('product_name', 'unknown')}: {str(e)}"
                )
                results["failed"] += 1
                results["errors"].append(str(e))
    ...
    def _is_quality_off_product(self, product: dict) -> bool:
        ...
        has_nutrition = any(
            product.get(f) for f in ["energy-kcal_100g", "proteins_100g", "carbohydrates_100g"]
        )
    ...
    async def _add_unit_mappings(self, item_id: int, category: str):
        ...
        try:
            ...
            category_info = self.food_service.category_unit_mappings.get(
                category, self.food_service.category_unit_mappings["other"]
            )
            ...
            for _i, unit in enumerate(category_info["allowed"]):
                ...
                self.db_service.execute_query(insert_query, (item_id, unit, is_primary, 0.8))
    ...
    async def _update_import_statistics(self, results: dict):
        ...
        try:
            ...
            metadata = {"errors": results.get("errors", []), "sources": list(results.keys())}

            self.db_service.execute_query(
                insert_query,
                (results["success"], results["skipped"], results["failed"], json.dumps(metadata)),
            )
...
async def main():
    ...
    parser.add_argument(
        "--sources", nargs="+", choices=["usda", "openfoodfacts"], help="Sources to import from"
    )
    parser.add_argument("--schedule", action="store_true", help="Run as scheduled service")
    parser.add_argument("--test", action="store_true", help="Run a test import with limited data")
