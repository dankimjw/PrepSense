...
class RecipeImportStats:
    ...
    def log_progress(self, batch_size: int = 100):
        ...
        if self.total_processed % batch_size == 0:
            ...
            logger.info(f"📊 Progress: {self.total_processed} processed, {self.successful_imports} imported, "
...
class IngredientParser:
    ...
    # Quantity patterns
    QUANTITY_PATTERN = r'^(\d+(?:\.\d+)?(?:\s*[-/]\s*\d+(?:\.\d+)?)?|\d+\s*\(\d+[^)]*\)|\½|⅓|⅔|¼|¾|⅛|⅜|⅝|⅞)'
    ...
    def _clean_ingredient_name(self, name: str) -> str:
        ...
        name = re.sub(r'^(fresh|dried|chopped|minced|sliced|diced|grated|frozen)\s+', '', name, flags=re.IGNORECASE)
        name = re.sub(r'\s+(fresh|dried|chopped|minced|sliced|diced|grated|frozen)$', '', name, flags=re.IGNORECASE)
...
class BackupRecipeImporter:
    ...
    async def import_recipes(self, batch_size: int = 100, start_from: int = 0, max_records: Optional[int] = None):
        ...
        try:
            ...
            logger.info(f"📋 Batch size: {batch_size}, Start from: {start_from}, Max records: {max_records}")
            ...
            logger.info(f"📊 Final stats: {self.stats.successful_imports} imported, {self.stats.failed_imports} failed")
    ...
    async def _insert_batch(self, conn: asyncpg.Connection, recipes: List[Dict], ingredients: List[Dict]):
        ...
        try:
            ...
            for recipe in recipes:
                recipe_id = await conn.fetchval(
                    ...
                    recipe['image_name'], recipe['cleaned_ingredients'],
                    recipe['prep_time'], recipe['cook_time'], recipe['servings'],
                    recipe['difficulty'], recipe['cuisine_type']
                )
            ...
            if ingredients and recipe_ids:
                ...
                for i, recipe_id in enumerate(recipe_ids):
                    ...
                    end_idx = start_idx + ingredients_per_recipe if i < len(recipe_ids) - 1 else len(ingredients)
