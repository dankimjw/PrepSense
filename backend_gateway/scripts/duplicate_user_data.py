...
def duplicate_user_data():
    ...
    try:
        ...
        user_data = db.execute_query(
            "SELECT * FROM users WHERE user_id = @user_id", {"user_id": SOURCE_USER_ID}
        )
        ...
        for username, new_user_id in user_id_map.items():
            ...
            existing_pantry = db.execute_query(
                "SELECT pantry_id FROM pantries WHERE user_id = @user_id", {"user_id": new_user_id}
            )
            ...
            if existing_pantry:
                ...
                result = db.execute_query(
                    ...
                    {
                        "user_id": new_user_id,
                        "pantry_name": f"{username}'s Pantry",
                        "created_at": datetime.now(),
                    },
                )
        ...
        source_pantry = db.execute_query(
            "SELECT pantry_id FROM pantries WHERE user_id = @user_id", {"user_id": SOURCE_USER_ID}
        )
