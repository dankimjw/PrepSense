...
def check_duplicate_pantry_items():
    ...
    if duplicate_examples:
        ...
        for dup in duplicate_examples[:10]:
            ...
            for item in dup['items']:
                print(f"    - ID: {item['pantry_item_id']}, Qty: {item['quantity']} {item['unit']}, Created: {item['created_at']}")
    ...
    for item in pacific_items:
        print(f"  User {item['user_id']}: ID {item['pantry_item_id']}, {item['quantity']} {item['unit_of_measurement']}")
