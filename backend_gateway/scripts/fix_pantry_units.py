        ...
        with conn.cursor() as cursor:
            for old_unit, new_unit in unit_mapping.items():
                ...
                if cursor.fetchone()[0] == 0:
                    print(f"  ⚠️  Skipping {old_unit} -> {new_unit} (target unit not in units table)")
                    continue
        ...
        with conn.cursor() as cursor:
            ...
            if unmapped_units:
                ...
                for unit, count in unmapped_units:
                    ...
                    if any(word in unit_lower for word in ['ml', 'liter', 'cup', 'gallon', 'pint', 'quart', 'fluid']):
                        suggested_unit = 'ml'
                    elif any(word in unit_lower for word in ['gram', 'kg', 'pound', 'lb', 'oz', 'ounce']):
                        suggested_unit = 'g'
                    elif any(word in unit_lower for word in ['bottle', 'can', 'jar', 'box', 'bag', 'package']):
                        suggested_unit = unit_lower if unit_lower in ['bottle', 'can', 'jar', 'box', 'bag', 'package'] else 'ea'
