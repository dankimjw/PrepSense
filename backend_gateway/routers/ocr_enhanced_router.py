...
from backend_gateway.routers.ocr_router import (
    ScanRequest, 
    ParsedItem, 
    OCRResponse,
    scan_receipt as original_scan_receipt,
    scan_items as original_scan_items
)
...
async def scan_receipt_enhanced(
    ...
    for idx, enhanced in enumerate(enhanced_results):
        ...
        if enhanced.get('enhanced', False):
            ...
            enhanced_item = EnhancedParsedItem(
                **original_item.dict(),
                matched_name=enhanced.get('cleaned_name'),
                enhanced=False
            )
