...
async def get_user_preferences(current_user: UserInDB = Depends(get_current_active_user)):
    ...
    try:
        ...
        if result:
            ...
            return PreferencesResponse(
                user_id=current_user.numeric_user_id, preferences=default_preferences, updated_at=""
            )
...
async def save_user_preferences(
    preferences: UserPreferences, current_user: UserInDB = Depends(get_current_active_user)
):
...
@router.delete("/")
async def clear_user_preferences(current_user: UserInDB = Depends(get_current_active_user)):
