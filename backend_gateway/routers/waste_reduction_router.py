...
async def analyze_pantry_waste_risk(user_id: str):
    ...
    try:
        ...
        high_risk_count = sum(1 for item in prioritized_items 
                             if item['risk_category'] in ['very_high', 'high'])
        
        total_potential_waste = sum(item.get('quantity_at_risk', 0) * item.get('base_loss_rate', 0) 
        ...
        return {
            ...
            "summary": {
                "immediate_action": [item['product_name'] for item in prioritized_items[:3] 
                                   if item['risk_category'] == 'very_high'],
                "use_soon": [item['product_name'] for item in prioritized_items[:5] 
...
async def get_waste_smart_recipes(
    user_id: str,
    recipe_ids: List[str] = Query(..., description="List of available recipe IDs")
):
    """
    ...
    try:
        ...
        high_risk_items = risk_analysis['summary']['immediate_action'] + risk_analysis['summary']['use_soon']
        
        if not high_risk_items:
            return {
                "message": "No high-risk items found in pantry",
                "prioritized_recipes": []
            }
        ...
        for recipe_id in recipe_ids[:10]:  # Limit to 10 recipes
            ...
            prioritized.append({
                ...
                "uses_high_risk_items": [item for item in high_risk_items 
                                        if item.lower() in recipe_name_lower]
            })
        ...
        return {
            ...
            "prioritized_recipes": prioritized[:5]
        }
...
async def get_waste_tracking_summary(
    ...
    try:
        ...
        daily_waste = postgres_service.execute_query(
            query, 
        ...
        return {
            ...
            "insights": {
                "trend": "decreasing" if len(daily_waste) > 1 and 
                        daily_waste[0]['total_waste_kg'] < daily_waste[-1]['total_waste_kg'] 
                        else "stable",
