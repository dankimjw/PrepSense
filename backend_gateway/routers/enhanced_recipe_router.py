...
class EnhancedRecipeSearchRequest(BaseModel):
    ...
    backup_min_match_ratio: float = Field(0.3, ge=0.0, le=1.0, description="Minimum ingredient match ratio for backup recipes")
...
class RecipeFromPantryRequest(BaseModel):
    ...
    enable_backup_recipes: bool = Field(True, description="Enable local backup recipe search")
    enable_spoonacular: bool = Field(True, description="Enable Spoonacular API search")
    backup_min_match_ratio: float = Field(0.4, ge=0.0, le=1.0, description="Minimum ingredient match ratio")


class RecipeSourceFilter(BaseModel):
    """Filter recipes by source."""
    sources: List[RecipeSource] = Field(default_factory=lambda: [RecipeSource.BACKUP_LOCAL, RecipeSource.SPOONACULAR])
...
async def search_recipes_from_pantry_enhanced(
    ...
    try:
        ...
        ingredients = [item.get('product_name', '').lower() for item in pantry_items if item.get('product_name')]
...
async def get_recipe_details_enhanced(
    recipe_id: str,
    source: Optional[RecipeSource] = Query(None, description="Recipe source (auto-detect if not provided)"),
...
async def get_performance_statistics(
    ...
    return {
        ...
        "recommendations": {
            ...
            "preferBackupRecipes": stats.get('backup_success_rate', 0) > stats.get('spoonacular_success_rate', 0)
        }
    }
