...
async def get_random_recipes(
    number: int = Query(20, ge=1, le=100, description="Number of random recipes"),
    tags: Optional[str] = Query(None, description="Comma-separated tags to filter by"),
    spoonacular_service: SpoonacularService = Depends(get_spoonacular_service)
) -> Dict[str, Any]:
    """
    Get random recipes for discovery - shows all recipes without filtering
    """
    try:
...
async def search_recipes_from_pantry(
    ...
    try:
        ...
        recipes = await spoonacular_service.search_recipes_by_ingredients(
            ...
            number=50,  # Increased to get more options
