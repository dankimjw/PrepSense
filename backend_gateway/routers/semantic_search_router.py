...
class HybridSearchRequest(BaseModel):
    ...
    semantic_weight: float = Field(
        0.6, ge=0.0, le=1.0, description="Weight for semantic similarity"
    )
    ingredient_weight: float = Field(
        0.4, ge=0.0, le=1.0, description="Weight for ingredient matching"
    )
...
async def hybrid_recipe_search(
    ...
    try:
        ...
        if abs(request.semantic_weight + request.ingredient_weight - 1.0) > 0.01:
            raise HTTPException(
                status_code=400, detail="Semantic weight and ingredient weight must sum to 1.0"
            )
...
async def find_similar_pantry_items(
    ...
    try:
        # Find similar items
        results = await db_service.find_similar_pantry_items(item_name=item_name, limit=limit)
...
async def update_entity_embedding(
    ...
    try:
        if entity_type not in ["recipe", "product"]:
            raise HTTPException(status_code=400, detail="Entity type must be 'recipe' or 'product'")
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404, detail=f"{entity_type.capitalize()} {entity_id} not found"
            )
