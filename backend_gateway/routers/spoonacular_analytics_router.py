    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "cache_hit_rate": len([c for c in target_calls if c.get('cache_hit', False)]) / len(target_calls) if target_calls else 0,
                "duplicate_detection_rate": len([c for c in target_calls if c.get('duplicate_detected', False)]) / len(target_calls) if target_calls else 0,
                "error_rate": len([c for c in target_calls if c.get('response_status', 200) >= 400]) / len(target_calls) if target_calls else 0,
                "unique_users": len(set(call.get('user_id') for call in target_calls if call.get('user_id'))),
...
async def get_user_usage(
    ...
    try:
        ...
        return {
            ...
            "summary": user_stats,
            "patterns": {
                "favorite_endpoints": dict(sorted(endpoint_preferences.items(), key=lambda x: x[1], reverse=True)),
                "daily_usage": daily_usage,
                "cost_efficiency": {
                    "total_recipes_found": total_recipes_found,
                    "total_cost_points": total_cost,
                    "cost_per_recipe": round(cost_per_recipe, 4),
                    "efficiency_rating": "high" if cost_per_recipe < 0.1 else "medium" if cost_per_recipe < 0.3 else "low"
                }
            },
            "recommendations": {
                "cache_utilization": "good" if user_stats.get('cache_hit_rate', 0) > 0.5 else "could_improve",
                "suggested_improvements": [
                    "Use bulk endpoints for multiple recipes" if endpoint_preferences.get('information', 0) > endpoint_preferences.get('informationBulk', 0) else None,
                    "Consider caching frequently accessed recipes" if user_stats.get('cache_hit_rate', 0) < 0.3 else None
                ]
            },
            "implementation_status": "🟡 PARTIAL - Endpoint working, database integration pending"
        }
...
async def get_cost_projection(
    ...
    try:
        ...
        # Usage growth trend
        growth_rate = ((daily_cost_7d - daily_cost_30d) / daily_cost_30d * 100) if daily_cost_30d > 0 else 0
        ...
        return {
            ...
            "recommendations": {
                "budget_status": "within_normal" if growth_rate < 20 else "increasing_rapidly" if growth_rate > 50 else "growing",
                "optimization_suggestions": [
                    "Increase cache hit rate to reduce API calls" if historical_stats_7d.get('cache_hit_rate', 0) < 0.6 else None,
                    "Enable duplicate detection to reduce redundant calls" if historical_stats_7d.get('duplicate_detection_rate', 0) < 0.1 else None,
                    "Consider bulk endpoints for multiple recipe requests" if cost_per_recipe > 0.2 else None
                ],
...
async def get_deduplication_stats(
    ...
    try:
        ...
        total_duplicates_removed = sum(len(call.get('duplicate_recipe_ids', [])) for call in dedup_calls)
        ...
        return {
            ...
            "summary": {
                ...
                "duplicate_rate": round(total_duplicates_removed / total_recipes_processed, 4) if total_recipes_processed > 0 else 0
            },
            "savings": {
                "estimated_cost_savings_points": estimated_cost_savings,
                "percentage_reduction": round((total_duplicates_removed / (total_recipes_processed + total_duplicates_removed)) * 100, 2) if total_recipes_processed > 0 else 0,
            ...
            "patterns": {
                ...
                "average_duplicates_per_call": round(total_duplicates_removed / dedup_call_count, 2) if dedup_call_count > 0 else 0
            },
            "effectiveness": {
                "algorithm_engagement": "high" if dedup_rate > 0.2 else "medium" if dedup_rate > 0.1 else "low",
                "savings_impact": "significant" if estimated_cost_savings > 100 else "moderate" if estimated_cost_savings > 20 else "minimal",
                "recommendation": "Continue using deduplication" if dedup_rate > 0.05 else "Review deduplication threshold settings"
            },
            "implementation_status": "🟡 PARTIAL - Endpoint working, database integration pending"
        }
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving deduplication stats: {str(e)}")
...
async def get_endpoint_performance(
    ...
    try:
        ...
        for endpoint, stats in endpoint_stats.items():
            ...
            cost_per_recipe = stats['total_cost'] / stats['total_recipes'] if stats['total_recipes'] > 0 else 0
            ...
            performance_summary[endpoint] = {
                ...
                'efficiency': {
                    'recipes_per_call': round(stats['total_recipes'] / call_count, 2) if call_count > 0 else 0,
                    'cost_per_recipe': round(cost_per_recipe, 4),
                    'cost_efficiency_rating': 'excellent' if cost_per_recipe < 0.1 else 'good' if cost_per_recipe < 0.5 else 'fair'
                }
            }
        ...
        return {
            ...
            "overall_summary": {
                ...
                "most_used_endpoint": max(endpoint_stats.keys(), key=lambda k: endpoint_stats[k]['call_count']) if endpoint_stats else None
            },
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving endpoint performance: {str(e)}")
...
async def analytics_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        feature_status = "🟢 WORKING - Database deployed" if db_table_exists else "🟡 PARTIAL - Endpoint working, database table pending"
        ...
        return {
            ...
            "components": {
                ...
                "deduplication_service": "available" if RecipeDeduplicationService else "unavailable"
            },
            "features": {
            ...
            "implementation_status": "🟡 PARTIAL - Endpoints working, database table deployment pending" if not db_table_exists else "🟢 WORKING - All components operational"
        }
