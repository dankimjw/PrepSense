...
@router.post("/add-from-recipe", response_model=Dict[str, Any], summary="Add Recipe Ingredients to Shopping List")
...
@router.get("/user/{user_id}/items/grouped", response_model=Dict[str, List[Dict[str, Any]]], summary="Get Shopping List Grouped by Category")
async def get_shopping_list_grouped(
    user_id: int,
    db_service = Depends(get_database_service)
):
    """
...
@router.post("/parse-ingredients", response_model=List[ParsedIngredient], summary="Parse Ingredient Strings")
async def parse_ingredients(
    ...
    try:
        ...
        for i, parsed_item in enumerate(parsed):
            result.append(ParsedIngredient(
                original_string=ingredients[i],
                parsed_name=parsed_item.get('aggregatable', {}).get('name', parsed_item.get('name', '')),
                quantity=parsed_item.get('aggregatable', {}).get('quantity', parsed_item.get('quantity', 0)),
                unit=parsed_item.get('aggregatable', {}).get('unit', parsed_item.get('unit', '')),
...
@router.post("/check-pantry-availability", response_model=Dict[str, Any], summary="Check Shopping List Against Pantry")
