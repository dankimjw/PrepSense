...
async def get_cooking_trends(
    ...
    try:
        ...
        cooking_events = db_service.execute_query(query, {
            'user_id': user_id,
        ...
        week_stats = db_service.execute_query(week_query, {
            'user_id': user_id,
            'start_date': week_ago
        })[0]
        
        month_stats = db_service.execute_query(week_query, {
            'user_id': user_id,
...
async def get_cooking_calendar(
    ...
    try:
        ...
        WHERE user_id = %(user_id)s
          ...
          AND recipe_name IS NOT NULL
          AND change_source = 'recipe'
        GROUP BY DATE(timestamp), recipe_name
        ORDER BY cook_date
        """
        
        events = db_service.execute_query(query, {
            'user_id': user_id,
            'start_date': start_date,
            'end_date': end_date
        })
        ...
        for event in events:
            ...
            calendar_data[date_str].append({
                ...
                'ingredients_used': event['ingredients_used']
            })
