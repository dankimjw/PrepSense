...
async def log_meal(request: LogMealRequest, user_id: int = Depends(get_current_user_id)):
        ...
        if True:  # Placeholder while service is disabled
            ...
            return LogMealResponse(
                ...
                daily_total=NutrientProfile(
                    calories=1200.0, protein=45.0, carbohydrates=150.0, fiber=18.0
                ),
...
async def get_nutrient_gaps(
    analysis_date: Optional[date] = Query(
        None, description="Date for analysis (defaults to today)"
    ),
    period_days: int = Query(
        1, description="Number of days to analyze (1 = single day, 7 = weekly)"
    ),
...
async def get_nutrient_aware_recipes(
    request: NutrientAwareRecipeRequest, user_id: int = Depends(get_current_user_id)
):
...
async def get_daily_intake(target_date: date, user_id: int = Depends(get_current_user_id)):
...
async def get_nutrition_goals(user_id: int = Depends(get_current_user_id)):
