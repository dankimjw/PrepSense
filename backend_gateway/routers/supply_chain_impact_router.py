...
router = APIRouter(
    prefix="/supply-chain-impact",
    tags=["supply chain impact"]
)
...
async def get_supply_chain_guide():
    ...
    try:
        ...
        avg_multiplier = sum(item['multiplier'] for item in multiplier_guide) / len(multiplier_guide)
...
async def get_weekly_trends(user_id: str, weeks: int = Query(4, description="Number of weeks to analyze")):
    """
    ...
    try:
        ...
        waste_data = db_service.execute_query(
            query, 
        ...
        for week_data in expired_data:
            ...
            weekly_trends.append({
                ...
                "waste_rate": round((items_wasted / items_expired * 100) if items_expired > 0 else 0, 1),
        ...
        return {
            ...
            "summary": {
                ...
                "prevention_rate": round((total_prevented / (total_prevented + total_wasted) * 100) if (total_prevented + total_wasted) > 0 else 0, 1),
                "total_co2e_saved": round(total_prevented_co2e, 1),
                "driving_miles_saved": round(total_prevented_co2e * 2.5, 0),
                "trend": "improving" if len(weekly_trends) > 1 and weekly_trends[0]['waste_rate'] < weekly_trends[-1]['waste_rate'] else "stable"
            }
        }
