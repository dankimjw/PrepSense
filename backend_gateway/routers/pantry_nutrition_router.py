...
async def get_item_nutrition(
    ...
    if not nutrition:
        raise HTTPException(
            status_code=404, detail="Nutritional information not available for this item"
        )
...
async def calculate_daily_intake(
    ...
    daily_nutrition = {
        nutrient: round(value / days, 2) for nutrient, value in summary["total_nutrition"].items()
    }
...
async def calculate_recipe_nutrition(
    ...
    async with db_pool.acquire() as conn:
        ...
        if not ingredients:
            raise HTTPException(status_code=404, detail="Recipe not found or has no ingredients")
        ...
        per_serving = {
            nutrient: round(value / servings, 2) for nutrient, value in total_nutrition.items()
        }
        ...
        return {
            ...
            "completeness": round(len(ingredients_with_nutrition) / len(ingredients) * 100, 1),
        }
