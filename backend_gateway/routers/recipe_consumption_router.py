...
async def cook_recipe(
    ...
    try:
        logger.info(
            f"User {request.user_id} cooking recipe {request.recipe_id}: {request.recipe_title}"
        )
        ...
        for ingredient in request.ingredients_used:
            try:
                ...
                if new_quantity < 0:
                    errors.append(
                        f"Insufficient {ingredient.ingredient_name}: need {ingredient.quantity_used}, have {current_quantity}"
                    )
                    continue
                ...
                if new_quantity <= 0:
                    ...
                    await pantry_service.update_pantry_item_quantity(
                        pantry_item_id=ingredient.pantry_item_id, new_quantity=new_quantity
                    )
        ...
        return {
            ...
            "errors": errors,
            "message": (
                f"Successfully cooked {request.recipe_title}"
                if len(errors) == 0
                else "Recipe cooked with some issues"
            ),
        }
...
async def check_ingredients_availability(
    ...
    try:
            ...
            if ingredient_key in pantry_lookup:
                ...
                for pantry_item in pantry_lookup[ingredient_key]:
                    ...
                    days_until_expiry = calculate_days_until_expiry(
                        pantry_item.get("expiration_date")
                    )
                    ...
                    key=lambda x: (
                        ...
                        -(
                            datetime.fromisoformat(
                                (x["created_at"] or "1970-01-01").replace("Z", "+00:00")
                            ).timestamp()
                        ),
                    )
                )
        ...
        return {"ingredients": ingredients_result}
...
async def quick_complete_recipe(
    ...
    try:
        ...
        for selection in request.ingredient_selections:
            try:
                ...
                if new_quantity <= 0:
                    ...
                    depleted_items.append(
                        {
                            ...
                            "pantry_item_name": pantry_item.get(
                                "product_name", selection.ingredient_name
                            ),
                        }
                    )
                else:
                    # Update remaining quantity
                    await pantry_service.update_pantry_item_quantity(
                        pantry_item_id=selection.pantry_item_id, new_quantity=new_quantity
                    )
                    updated_items.append(
                        {
                            "item_id": selection.pantry_item_id,
        ...
        success = len(errors) == 0
        message = (
            f"Successfully completed {recipe_title}!"
            if success
            else f"Completed {recipe_title} with some issues"
        )
