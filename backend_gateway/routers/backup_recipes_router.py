...
class BackupRecipeSearchResult:
    ...
    def __init__(self, db_row: dict):
        ...
        self.image = f"/api/v1/backup-recipes/images/{db_row.get('image_name')}" if db_row.get('image_name') else None
        self.imageType = "jpg"
...
async def search_backup_recipes(
    ...
    ingredients: Optional[str] = Query(None, description="Comma-separated list of available ingredients"),
    ...
    fillIngredients: bool = Query(False, description="Add information about used and missed ingredients"),
    ...
    try:
        ...
        if ingredients and fillIngredients:
            order_clause = "ORDER BY ingredient_matches.match_ratio DESC, (prep_time + cook_time) ASC"
        elif sort == "time":
    ...
    try:
        ...
        if tags:
            ...
            where_clause = "WHERE " + " OR ".join([
                f"title ILIKE '%{tag}%' OR cuisine_type ILIKE '%{tag}%'"
                for tag in tag_list
            ])
...
@router.get("/stats")
async def get_backup_recipe_stats(
    pool: asyncpg.Pool = Depends(get_db_pool)
) -> Dict[str, Any]:
    ...
    try:
        async with pool.acquire() as conn:
            ...
            return {
                ...
                "avg_ingredients_per_recipe": round(ingredient_stats['avg_ingredients_per_recipe'] or 0, 1),
...
@router.post("/reindex")
async def reindex_search_vectors(
    pool: asyncpg.Pool = Depends(get_db_pool)
) -> Dict[str, str]:
