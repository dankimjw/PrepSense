...
class ChatStreamingService:
    ...
    async def process_message_stream(self, message: str, user_id: int, 
    ...
    def _is_recipe_request(self, message: str) -> bool:
        ...
        recipe_keywords = [
            'recipe', 'cook', 'make', 'dinner', 'lunch', 'breakfast', 'meal',
            'ingredient', 'food', 'eat', 'prepare', 'dish', 'kitchen'
        ]
        
        message_lower = message.lower()
        return any(keyword in message_lower for keyword in recipe_keywords)
    
    async def _stream_recipe_recommendations(self, message: str, user_id: int, 
    ...
    async def _stream_general_response(self, message: str, user_id: int, 
                                     context: Dict[str, Any]) -> AsyncGenerator[Dict[str, Any], None]:
        """Stream general chat response"""
...
async def clear_cache(
    cache_type: str = Query(default='all', description="Type of cache to clear: all, inventory, expiry, preferences"),
    db_service = Depends(get_database_service)
):
    """Clear cache data"""
    try:
