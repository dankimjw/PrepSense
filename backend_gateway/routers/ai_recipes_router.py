...
async def generate_ai_recipes(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error generating AI recipes: {str(e)}"
        )
...
async def get_pantry_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting pantry summary: {str(e)}"
        )
...
async def generate_ai_recipes_async(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error starting recipe generation: {str(e)}"
        )
