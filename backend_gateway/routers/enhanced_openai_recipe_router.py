...
class RecipeGenerationRequest(BaseModel):
    ...
    dietary_restrictions: Optional[list[str]] = Field(
        None, description="Dietary restrictions (vegetarian, vegan, etc.)"
    )
...
class BatchRecipeRequest(BaseModel):
    ...
    recipe_requests: list[RecipeGenerationRequest] = Field(
        ..., description="List of recipe generation requests"
    )
...
@router.post(
    "/generate", response_model=RecipeResponse, summary="Generate a single enhanced recipe"
)
async def generate_enhanced_recipe(request: RecipeGenerationRequest) -> RecipeResponse:
    """
    ...
    try:
        ...
        return RecipeResponse(success=True, recipe=recipe, validation_result=validation_result)

    except Exception as e:
        logger.error(f"Error generating enhanced recipe: {str(e)}")
        return RecipeResponse(success=False, error=str(e))


@router.get(
    "/generate-from-query",
    response_model=RecipeResponse,
    summary="Generate recipe from query parameters",
)
async def generate_recipe_from_query(
    ...
    try:
        ...
        dietary_list = (
            [diet.strip() for diet in dietary_restrictions.split(",") if diet.strip()]
            if dietary_restrictions
            else None
        )
        allergen_list = (
            [allergen.strip() for allergen in allergens.split(",") if allergen.strip()]
            if allergens
            else None
        )
...
@router.post(
    "/generate-batch",
    response_model=BatchRecipeResponse,
    summary="Generate multiple enhanced recipes",
)
async def generate_batch_recipes(request: BatchRecipeRequest) -> BatchRecipeResponse:
    """
    ...
    try:
        ...
        recipes = enhanced_recipe_service.batch_generate_recipes(
            recipe_requests=recipe_requests, max_concurrent=request.max_concurrent
        )
...
async def validate_recipe_structure(recipe: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    return {
        ...
        "template": {
            ...
            "nutrition": {
                ...
                "caloricBreakdown": {"percentProtein": 30, "percentFat": 26, "percentCarbs": 44},
                "calories": 420,
                "protein": 32,
                "carbs": 45,
                "fat": 12,
...
async def health_check() -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "service_status": "unhealthy", "error": str(e)}
