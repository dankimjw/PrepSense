...
from backend_gateway.crewai.tools.monitoring_tool import (
    get_all_tool_stats,
...
async def reset_circuit_breaker(
    ...
    try:
        ...
        logger.info(
            f"Circuit breaker reset for agent",
            extra={
...
async def get_active_executions(
    ...
    try:
        ...
        for task_id, context in observability_manager.active_executions.items():
        ...
        raise HTTPException(status_code=500, detail=f"Active executions retrieval failed: {str(e)}")
...
async def get_agent_debug_info(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Debug info retrieval failed: {str(e)}")
...
async def get_tool_statistics() -> JSONResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Tool statistics failed: {str(e)}")
        ...
        dashboard_data = {
            ...
            "agent_status": {
                agent_type.value: {
                    ...
                    "circuit_breaker": health_data["agent_health"][agent_type.value]["circuit_breaker"]["state"]
                }
