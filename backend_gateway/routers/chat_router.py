...
async def generate_recipe_image(request: ImageGenerationRequest):
    """
    ...
    try:
            ...
            if openai.api_key:
                try:
                    ...
                    return ImageGenerationResponse(
                        image_url=image_url, recipe_name=request.recipe_name
                    )
                except Exception as e:
                    logger.warning(
                        f"DALL-E 2 generation failed, falling back to Unsplash: {str(e)}"
                    )
        ...
        if not unsplash_access_key:
            ...
            return ImageGenerationResponse(image_url=image_url, recipe_name=request.recipe_name)
        ...
        return ImageGenerationResponse(
            image_url=fallback_images[image_index], recipe_name=request.recipe_name
        )
