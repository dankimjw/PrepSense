...
class PantryNormalizationRequest(BaseModel):
    ...
    image_b64: Optional[str] = Field(None, description="Base64 encoded image for BiteCam processing")
    raw_items: Optional[List[Dict[str, Any]]] = Field(None, description="Pre-extracted raw items [{'raw_line': '2 lb chicken'}]")
...
class PrepSenseWorkflowRequest(BaseModel):
    ...
    # Health goals
    user_health_goals: Optional[Dict[str, Any]] = Field(None, description="User health goals for nutrition analysis")
    
    # Recipe selection
    selected_recipe_id: Optional[int] = Field(None, description="Recipe to deduct ingredients for")
    commit_deduction: bool = Field(False, description="Whether to actually deduct ingredients from pantry")
...
def get_main_crew() -> PrepSenseMainCrew:
    ...
    if main_crew is None:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to initialize CrewAI system: {str(e)}")
...
def get_pantry_crew() -> PantryNormalizationCrew:
    ...
    if pantry_crew is None:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to initialize pantry normalization: {str(e)}")
...
async def select_recipe(
    ...
    try:
        ...
        if commit_deduction:
            ...
            logger.info(f"Checking ingredient availability for recipe {recipe_id} for user {user_id}")
...
async def test_imports():
    ...
    try:
        ...
        return {
            ...
            "agents": [
                "BiteCam", "FoodCategorizer", "UnitCanon", "FreshFilter",
                "RecipeSearch", "NutriCheck", "UserPreferences", "JudgeThyme", "PantryLedger"
            ],
