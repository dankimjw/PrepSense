...
async def get_food_environmental_impact(food_name: str):
    ...
    try:
        ...
        if not impact_data:
            raise HTTPException(
                status_code=404, detail=f"No environmental data found for {food_name}"
            )
    ...
    try:
        ...
        for ing in ingredients:
            ...
            if len(parts) >= 2:
                ...
                parsed_ingredients.append({"name": name, "quantity_kg": quantity_kg})
...
async def get_sustainable_swaps(
    ingredient: str, max_suggestions: int = Query(5, description="Maximum number of suggestions")
):
    """
    ...
    try:
        ...
        return {"ingredient": ingredient, "suggestions": swaps}
...
async def analyze_pantry_sustainability(user_id: str):
    ...
    try:
        ...
        pantry_items = postgres_service.execute_query(query, {"user_id": user_id})
        ...
        return {"user_id": user_id, "analysis": analysis, "report": report}
...
async def get_product_eco_score(product_id: str):
    ...
    try:
        ...
        product = postgres_service.execute_query(query, {"product_id": product_id}, fetch="one")
