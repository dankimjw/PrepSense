...
from .models import PantryArtifact, PantryState, PreferenceArtifact, PreferenceState, RecipeState
...
class PantryAnalysisFlow(Flow[PantryState]):
    ...
    def create_vectors(self, previous_result: str) -> str:
        ...
        try:
            ...
            self.state.ingredient_vectors = create_ingredient_embeddings(
                self.state.normalized_items
            )
        except ImportError:
    ...
    def _clean_item_name(self, name: str) -> str:
        ...
        cleaned = re.sub(
            r"\s*[–-]\s*\d+\s*(lbs?|oz|gal|bag|pack|count|units?).*$", "", name, flags=re.IGNORECASE
        )
...
class PreferenceLearningFlow(Flow[PreferenceState]):
    ...
    def gather_interactions(self) -> str:
        ...
        try:
            self.state.user_interactions = self.db_service.execute_query(
...
class RecipeIntelligenceFlow(Flow[RecipeState]):
    ...
    def update_recipe_embeddings(self) -> str:
        ...
        try:
            ...
            # Mock embeddings for testing
            recipe_embeddings = {recipe["id"]: [0.1, 0.2, 0.3] for recipe in recipes}
