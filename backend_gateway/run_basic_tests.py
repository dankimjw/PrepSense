...
def main():
    ...
    pytest_available = run_command(
        [sys.executable, "-m", "pytest", "--version"], "Pytest version check"
    )
    ...
    if pytest_available:
        ...
        test_results["basic_tests"] = run_command(
            [
                ...
                "-k",
                "not (test_db or testcontainers or test_migrations)",
            ],
            "Basic pytest tests",
        )
    ...
    test_results["syntax"] = run_command(
        [sys.executable, "-m", "py_compile", "services/recipe_advisor_service.py"],
