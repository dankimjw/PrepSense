...
class NutritionTool:
    ...
    def _compare_to_rda(self, **kwargs) -> Dict[str, Any]:
        ...
        for nutrient, value in nutrition.items():
            if nutrient in self.rda_values and isinstance(value, (int, float)):
                ...
                if nutrient in ['sodium', 'sugar', 'saturated_fat', 'cholesterol'] and percentage > 120:
                    excesses.append({
                        'nutrient': nutrient,
                        'percentage': percentage,
    ...
    def _evaluate_nutritional_balance(self, **kwargs) -> Dict[str, Any]:
        ...
        if 10 <= macro_percentages['protein'] <= 35:
            ...
            recommendations.append(f"Adjust protein intake (currently {macro_percentages['protein']}%, ideal 10-35%)")
        
        if 45 <= macro_percentages['carbohydrates'] <= 65:
            balance_score += 1
        else:
            recommendations.append(f"Adjust carbohydrate intake (currently {macro_percentages['carbohydrates']}%, ideal 45-65%)")
        
        if 20 <= macro_percentages['fat'] <= 35:
            balance_score += 1
        else:
            recommendations.append(f"Adjust fat intake (currently {macro_percentages['fat']}%, ideal 20-35%)")
    ...
    def _generate_gap_recommendations(self, gaps: List[Dict[str, Any]]) -> List[str]:
        ...
        for gap in gaps:
            ...
            if nutrient == 'protein':
                recommendations.append(f"Add {gap_amount:.0f}g more protein (lean meats, beans, nuts)")
            elif nutrient == 'fiber':
                recommendations.append(f"Add {gap_amount:.0f}g more fiber (whole grains, vegetables, fruits)")
            elif nutrient == 'vitamin_c':
                recommendations.append(f"Add {gap_amount:.0f}mg more vitamin C (citrus fruits, berries)")
            elif nutrient == 'calcium':
                recommendations.append(f"Add {gap_amount:.0f}mg more calcium (dairy, leafy greens)")
            elif nutrient == 'iron':
                recommendations.append(f"Add {gap_amount:.0f}mg more iron (red meat, spinach, legumes)")
