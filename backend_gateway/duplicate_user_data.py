...
async def duplicate_user_data():
    ...
    try:
        ...
        # Get user profile
        user_data = await db.fetch_one("SELECT * FROM users WHERE user_id = $1", SOURCE_USER_ID)
        ...
        for new_user in NEW_USERS:
            ...
            existing = await db.fetch_one(
                "SELECT user_id FROM users WHERE user_id = $1", new_user["id"]
            )
        ...
        for new_user in NEW_USERS:
            pantry_count = await db.fetch_one(
                "SELECT COUNT(*) as count FROM pantry_items WHERE user_id = $1", new_user["id"]
            )
            ...
            favorite_count = await db.fetch_one(
                "SELECT COUNT(*) as count FROM recipe_favorites WHERE user_id = $1", new_user["id"]
            )
