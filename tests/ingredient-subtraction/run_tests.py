...
class TestRunner:
    ...
    def run_test(self, test_name: str, endpoint: str, payload: Dict[str, Any], 
                 validations: List[callable]) -> Dict[str, Any]:
...
def validate_updated_items(expected_count: int):
    def validate(response: Dict) -> tuple[bool, str]:
        ...
        return count == expected_count, f"Updated items count: expected {expected_count}, got {count}"
    return validate
    ...
    def validate(response: Dict) -> tuple[bool, str]:
        for item in response.get("updated_items", []):
            if item_name.lower() in item.get("item_name", "").lower():
                ...
                return abs(actual - expected_new_quantity) < 0.01, \
...
def validate_missing_items(expected_items: List[str]):
    def validate(response: Dict) -> tuple[bool, str]:
        ...
        return set(expected_items) == set(missing), \
...
def main():
    ...
    runner.run_test(
        ...
        [
            validate_updated_items(0),
            validate_missing_items(["saffron"])
        ]
    )
    ...
    runner.run_test(
        ...
        [
            validate_updated_items(1),
            validate_item_quantity("Tomato", 2)
        ]
    )
    ...
    runner.run_test(
        ...
        [
            validate_updated_items(1),
            validate_item_quantity("Salt", 0)  # Should use all available
        ]
    )
    ...
    runner.run_test(
        ...
        [
            lambda r: (len(r.get("updated_items", [])) >= 4, 
    ...
    runner.run_test(
        ...
        [
            lambda r: ("reverted_items" in r and len(r["reverted_items"]) > 0,
